...
import numpy as np
import itertools
from convexhull import get_hull
...
def initializeGrid(size, dimensions, random_seed):
    ...
    indices = np.random.choice(grid.shape[0], replace=False,
                               size=int(grid.shape[0] * 0.8))
    grid[indices] = size + 1
...
if __name__ == '__main__':
    ...
    print("----")
    print("Agent position : ", env.pos)

    st, r, done, _ = env.step(0)
    print("Data : ", st, r, done)
    st, r, done, _ = env.step(0)
    print("Data : ", st, r, done)

    terminal_states = env.terminal_list()
    ch = get_hull(-1 * np.array(terminal_states))
    print(terminal_states)
    print("Now the convex hull")
    print(ch, len(ch))
    print("Now the pareto front")
    print(env.pareto_front, len(env.pareto_front))
    #print(env.render())

    if d == 3:
        voxel_representation(env, s, d)
