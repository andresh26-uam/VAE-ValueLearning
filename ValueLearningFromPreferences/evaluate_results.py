        ...
        for cluster_idx in range(assignments[0].L):
            ...
            for assignment in assignments:
                sorted_clusters = sorted(
                    range(assignments[0].L), key=lambda idx: -len(assignment.assignment_vs[idx]))
            ...
            if output_columns.get("num_agents", False):
                num_agents = [len(assignment.assignment_vs[cidx]) for assignment, cidx in zip(
            ...
            if output_columns.get("representativity", False):
                representativities = [
                    ClusterAssignment._representativity_cluster(
                        ...
                        ) if agent in assignment.assignment_vs[cidx]]
...
def plot_metrics_for_experiments(historic_assignments_per_lre: Dict[str, List[ClusterAssignment]], experiment_names, assignment_memories: Dict[str, ClusterAssignmentMemory], n_iterations_real=500, fontsize=16):
    ...
    for idx, ename in enumerate(experiment_names):
        ...
        ), assignment_memory.maximum_conciseness_vs) for assignment in historic_assignments]
...
if __name__ == "__main__":
    ...
    if hasattr(parser_args, 'learning_curve_from') and parser_args.learning_curve_from is not None:
        ...
        for ename in parser_args.learning_curve_from:
            ...
            assignment_memories_per_lre[ename_clean].sort_lexicographic(
    ...
    if hasattr(parser_args, 'dunn_index_curve_from') and parser_args.dunn_index_curve_from is not None:
        ...
        for best in best_per_memory.values():
            ...
            scores[key].append(best.combined_cluster_score_vs(
                aggr_repr=np.min, conciseness_if_L_is_1=max_conciseness if max_conciseness > 0 else None))
    ...
    society_config_actual = load_json_config(exp_parser_args.society_file if hasattr(
        exp_parser_args, 'society_file') else 'societies.json')
    ...
    example_model: LinearVSLRewardFunction = list(
        reward_net_pair_agent_and_vs.values())[0]
    ...
    vsl_algo.init_models(10, vsl_algo.vsi_optimizer_cls,
                         vsl_algo.vsi_optimizer_kwargs)
    # pprint.pprint(config[exp_parser_args.environment])
    ...
    if plot_test:
        test_assignment_memory = ClusterAssignmentMemory(
            assignment_memory.max_size, n_values=assignment_memory.memory[0].n_values)
        test_assignment_memory.maximum_conciseness_vs = assignment_memory.maximum_conciseness_vs
        # these two are train estimations.
        test_assignment_memory.maximum_conciseness_gr = assignment_memory.maximum_conciseness_gr
        for a in assignment_memory.memory:
            test_assignment = vsl_algo.evaluate_assignment(a, dataset_test)
            # just insert it.
            test_assignment_memory.memory.append(test_assignment)
        ...
        best_vs_then_gr_assignment_test = vsl_algo.evaluate_assignment(
            best_vs_then_gr_assignment, dataset_test)
        best_gr_then_vs_assignment_test = vsl_algo.evaluate_assignment(
            best_gr_then_vs_assignment, dataset_test)
    ...
    generate_assignment_tables(assignments_identifier_to_assignment, experiment_name, output_columns,
                               output_dir='test_results', label='train_set', values_names=values_short_names)
    if plot_test:
        generate_assignment_tables(test_assignments_identifier_to_assignment, experiment_name,
                                   output_columns, output_dir='test_results', label='test_set', values_names=values_short_names)
    # For the list of assignments with different seeds_
    if hasattr(parser_args, 'learning_curve_from') and parser_args.learning_curve_from is not None:
        generate_assignment_tables(assignments_identifier_to_assignment_lre, experiment_name, output_columns,
                                   output_dir='test_results', label='train_set', values_names=values_short_names)
        if plot_test:
            generate_assignment_tables(assignments_identifier_to_assignment_lre, experiment_name,
                                       output_columns, output_dir='test_results', label='test_set', values_names=values_short_names)

    # 3: Explainability
    if isinstance(env, RouteChoiceEnvironmentApolloComfort):
        if plot_test:
            data_test = np.array([[*t1.obs[0], *t2.obs[0]] for (t1, t2)
                                 in zip(dataset_test.fragments1, dataset_test.fragments2)])
        data_train = np.array([[*t1.obs[0], *t2.obs[0]] for (t1, t2)
                              in zip(dataset_train.fragments1, dataset_train.fragments2)])
    else:
        env: TabularVAMDP
    ...
    for i, (data_train_or_test, dataset_train_or_test) in enumerate(zip([data_train, data_test], [dataset_train, dataset_test])):
        ...
        feature_names = ['Time T1', 'Cost T1', 'Headway T1', 'Interchanges T1',
                         'Time T2', ' Cost T2', 'Headway T2', 'Interchanges T2']
        ...
        for value in range(dataset_train.n_values):
            ...
            reward_model_per_value.append(lambda obs: vsl_algo.preference_model.predict_proba(fragment_pairs=obs_shap_to_fragment_pairs(obs),
                                                                                              ...
                                                                                              obs_acts_next_obs_idxs=obs_acts_next_obs_idxs, alignment=vsl_algo.env.basic_profiles[
                                                                                                  value],
                                                                                              model=best_vs_then_gr_assignment.reward_model_per_agent_id[agent]))

            s = b.MorrisSensitivity(
                data=data_train_or_test, model=reward_model_per_value[value], feature_names=feature_names)
            ...
            explanations_dir = os.path.join(
                'test_results', experiment_name, 'train_set' if i == 0 else 'test_set', 'explanations')
            morris_dir = os.path.join(explanations_dir, "morris")
            os.makedirs(morris_dir, exist_ok=True)
            path = os.path.join(
                morris_dir, f"morris_{values_names[value]}.pdf")
