...
def parse_policy_approximator(ref_class, env_name: str, society_data: Dict, environment_data: Dict, ref_policy_kwargs: Dict, environment: gym.Env, 
    ...
    #exit(0)
    return ref_class, ref_policy_kwargs
...
if __name__ == "__main__":
    ...
    policy = epclass(**epkwargs)
    policy.train(total_timesteps=2000, eval_env=environment, ref_point=np.array([-100, -100]))
    policy.set_desired_return_and_horizon(desired_return=[10000]*environment_data['n_values'], desired_horizon=environment_data['horizon'])
    for w in agent_profiles:
        if epclass != LearnerValueSystemLearningPolicy:
            try:
                otrajs, evaluation = obtain_trajectories_and_eval_mo(
                    agent=policy, env=environment, n_seeds=100, ws=[w], ws_eval=[w], seed=DEFAULT_SEED, exploration=0.0, repeat_per_seed=1, agent_name='unknown')
                
                m_calc_r = np.mean([t.vs_rews.sum() for t in otrajs])
                print("O trajs mean reward: ", m_calc_r)
                print(f"Evaluation for {w}: {evaluation}")
                assert m_calc_r == evaluation[0]
