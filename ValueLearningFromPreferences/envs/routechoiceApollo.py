...
class RouteChoiceEnvironmentApollo(TabularVAMDP):
    def __init__(self, value_columns: List[int] = (2, 3, 4, 5), test_size=0.2, random_state=42):
        ...
        with open(DATASET_FILE, 'r') as f:
            ...
            # Assuming 'ID' is the column for agent IDs
            for agent_id, group in data.groupby('ID'):
                if test_size > 0:
        ...
        self.basic_profiles = [tuple(pr) for pr in np.eye(
            self.n_values, dtype=np.float32)]

        self.routes_train, self.routes_test, observation_matrix, self._reward_matrix_per_va, agent_context_matrix, self.preferences_per_agent_id, self.preferences_grounding_per_agent_id = self._parse_trajectory_data_from_pd_dataset(
    ...
    def compare_groundings(self, first_obs, second_obs, vi):
        # This is the default rule: smaller cost/time/headway/interchanges the better
        return 1.0 if -first_obs[vi] + second_obs[vi] > 0 else 0.0
    ...
    def _parse_trajectory_data_from_pd_dataset(self, train_dataset: np.ndarray, test_dataset: np.ndarray) -> List[TrajectoryWithValueSystemRews]:
        ...
        preferences_grounding_per_agent_id = {
            ...
            for row in dataset:
                ...
                for first_or_second, obs in enumerate([observations1, observations2]):
                    ...
                    if first_or_second == 0:
                        ...
                        preferences_per_agent_id[agent_id][(
                            first_pair_id, second_pair_id)] = 1.0-(choice-1)
                        for vi in range(self.n_values):
                            preferences_grounding_per_agent_id[vi][agent_id][(
                                first_pair_id, second_pair_id)] = self.compare_groundings(first_obs, second_obs, vi)
                    trajectory = TrajectoryWithValueSystemRews(
                        ...
                        v_rews=np.array([[self._reward_ground_truth(
                            obs, ipr) for ipr in range(self.n_values)]]).transpose(),
                        rews=np.array([1.0 if (choice == 1 and first_or_second == 0) or (
                            choice == 2 and first_or_second == 1) else 0.0]),
                    ...
                    if first_or_second == 0 and choice == 1:
                        ...
                        assert preferences_per_agent_id[agent_id][(
                            first_pair_id, second_pair_id)] == 1.0
                    elif first_or_second == 1 and choice == 2:
                        assert trajectory.rews[0] == 1.0
                        assert trajectory.infos[0]['state'] == second_pair_id
                        assert preferences_per_agent_id[agent_id][(
                            first_pair_id, second_pair_id)] == 0.0
        ...
        _reward_matrix_per_va = {tuple(prof): np.zeros(
            (new_id + 1, 1)) for prof in self.basic_profiles}
        ...
        for traj in itertools.chain(trajectories_train, trajectories_test):
            ...
            for ipr, prof in enumerate(self.basic_profiles):
                ...
                _reward_matrix_per_va[prof][traj.infos[0]['state'],
                                            0] = self._reward_ground_truth(traj.obs[0], ipr)
            agent_context_matrix[traj.infos[0]['state']
                                 ] = traj.infos[0]['agent_context']
...
class RouteChoiceEnvironmentApolloComfort(RouteChoiceEnvironmentApollo):
    def __init__(self, value_columns: List[int] = (2, 3, 4), test_size=0.2, random_state=42):
        super().__init__(value_columns=value_columns,
                         test_size=test_size, random_state=random_state)

    def compare_groundings(self, first_obs, second_obs, vi):
        if vi < 2:
            # This is the default rule: smaller time/cost/headway/interchanges the better
            ret = 1.0 if -first_obs[vi] + second_obs[vi] > 0 else 0.0
        else:
            case1better = - \
                first_obs[2] + second_obs[2] > 0 and - \
                first_obs[3] + second_obs[3] >= 0.0
            case1better2 = - \
                first_obs[2] + second_obs[2] >= 0 and - \
                first_obs[3] + second_obs[3] > 0.0
            caseworse1 = - \
                first_obs[2] + second_obs[2] < 0 and - \
                first_obs[3] + second_obs[3] <= 0
            caseworse2 = - \
                first_obs[2] + second_obs[2] <= 0 and - \
                first_obs[3] + second_obs[3] < 0
    ...
    def _reward_ground_truth(self, obs_preprocessed, value_idx):
        # this is comfort, overriden by the previous method!
        return -obs_preprocessed[value_idx] if value_idx < 2 else -abs(obs_preprocessed[2]*(obs_preprocessed[3] + 1))
