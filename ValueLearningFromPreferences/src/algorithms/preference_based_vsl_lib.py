...
def calculate_accuracies(probs_vs: th.Tensor = None, probs_gr: th.Tensor = None, gt_probs_vs: th.Tensor = None, gt_probs_gr: th.Tensor = None, indifference_tolerance=0.0, reduce='mean'):
    ...
    if probs_vs is not None:
        ...
        misclassified_vs = ~ (
            wellclassified_positive | wellclassified_negative | wellclassified_indifferent)
    ...
    if probs_gr is not None:
        ...
        for j in range(probs_gr.shape[-1]):
            ...
            if reduce == 'mean':
                acc_gr_vi = (1.0 - sum(missclassified_vgrj) /
                             len(detached_probs_vgrj))
            else:
...
def total_variation_distance(preferences1, preferences2):
    ...
    # /len(preferences1)
    return th.mean(th.square(preferences1 - preferences2))


def jensen_shannon_pairwise_preferences(preferences1, preferences2):
    # return th.norm(preferences1-preferences2, p=p)
    m = (preferences1+preferences2)/2.0
    k1 = preferences1*(th.log(preferences1+1e-8) - th.log(m+1e-8)) + \
        (1-preferences1)*(th.log(1-preferences1+1e-8) - th.log(1-m+1e-8))
    k2 = preferences2*(th.log(preferences2+1e-8) - th.log(m+1e-8)) + \
        (1-preferences2)*(th.log(1-preferences2+1e-8) - th.log(1-m+1e-8))
...
def likelihood_x_is_target(pred_probs, target_probs, mode='numpy', slope=1, adaptive_slope=True, qualitative_mode=False, indifference_tolerance=0.0):
    ...
    if qualitative_mode:
        ...
        probs[(abs(probs - target_probs) <= indifference_tolerance)
...
class PreferenceModelClusteredVSL(preference_comparisons.PreferenceModel):
    ...
    def conciseness_pairwise_gr(self,
        ...
        cm_with_different_grounding1.set_network_for_value(
            value_idx, grounding1)
        cm_with_different_grounding2 = self.dummy_models[1]
        cm_with_different_grounding2.set_network_for_value(
            value_idx, grounding2)
        ...
        probs, _ = self.forward(fragment_pairs_per_agent_id=fragments_dict, fragment_pairs_idxs_per_agent_id=None,
                                ...
                                custom_model_per_agent_id={
                                    'g1': cm_with_different_grounding1, 'g2': cm_with_different_grounding2},
        ...
        disc = discordance(probs=probs_with_cluster1, gt_probs=probs_with_cluster2,
                           indifference_tolerance=indifference_tolerance)
    ...
    def predict_proba(
        ...
        obs = np.concatenate(
            (obs1, fragment_pairs[1][:, obs_acts_next_obs_idxs['obs']]), axis=0)
        acts = np.concatenate((fragment_pairs[0][:, obs_acts_next_obs_idxs['acts']], fragment_pairs[1][:,
                              obs_acts_next_obs_idxs['acts']]), axis=0) if model.use_action else np.zeros((len(obs),), dtype=obs1.dtype)
        next_obs = np.concatenate((fragment_pairs[0][:, obs_acts_next_obs_idxs['next_obs']], fragment_pairs[1]
                                  [:, obs_acts_next_obs_idxs['next_obs']]), axis=0) if model.use_next_state else obs
        dones = np.concatenate((fragment_pairs[0][:, obs_acts_next_obs_idxs['dones']], fragment_pairs[1][:,
                               obs_acts_next_obs_idxs['dones']]), axis=0) if model.use_done else np.zeros((len(acts),), dtype=np.bool_)
        ...
        for aid, fragment_pairs_aid in fragment_pairs_per_agent_id.items():
            ...
            if idx_global == 0:
                if return_rewards_global:
                    rews_vs = (th.empty(total_number_of_fragment_pairs, fragment_size, dtype=dtype), th.empty(
                        total_number_of_fragment_pairs, fragment_size, dtype=dtype))
                    rews_gr = (th.empty(
            ...
            if self.algorithm.allow_variable_horizon:
                raise ValueError(
                    "You should overhaul this... Like padding with 0 maybe?")
                ...
                if not only_grounding:
                    ...
                    if return_rewards_global:
                        ...
                        rews_vs[0][idx_global:idx_global +
                                   len(rews1_vsaid_all), :] = rews1_vsaid_all
                        rews_vs[1][idx_global:idx_global +
                                   len(rews2_vsaid_all), :] = rews2_vsaid_all
                ...
                if only_for_alignment_function is None:
                    for j in range(n_values):
                        ...
                        if return_rewards_global:
                            rews_gr[0][idx_global:idx_global +
                                       len(rews1_graid_all_j), :, j] = rews1_graid_all_j
                            rews_gr[1][idx_global:idx_global +
                                       len(rews2_graid_all_j), :, j] = rews2_graid_all_j
                        if return_rewards_per_agent:
                            rews_gr_per_aid[aid][0][:,
                                                    :, j] = rews1_graid_all_j
                            rews_gr_per_aid[aid][1][:,
                                                    :, j] = rews2_graid_all_j
                            assert rews_gr_per_aid[aid][0].shape == (
                                len(fragment_pairs_aid), fragment_size, n_values)
                idx_global += len(rews1_vsaid_all)
            with th.no_grad():
                if fragment_pairs_idxs_per_agent_id is not None:
                    # TODO optimize later to index rews_gr_per_aid
                    fragment_idxs = fragment_pairs_idxs_per_agent_id[aid]
                else:
                    # just put them in order of appearance of each agent.
                    fragment_idxs = np.array(
                        list(range(n_fragments))) + counter_idx
            if not only_grounding:
    ...
    def rewards(self, transitions: Transitions, only_with_alignment=False, only_grounding=False, real=False, alignment=None, grounding=None, custom_model=None, reward_mode=None) -> th.Tensor:
        ...
        rews = th.zeros((len(transitions.obs), ),
                        device=self.model.device, dtype=self.model.dtype)
        rews_gr = th.zeros((self.algorithm.env.n_values, len(
            transitions.obs)), device=self.model.device, dtype=self.model.dtype)
...
class BasicRewardTrainerVSL(preference_comparisons.BasicRewardTrainer):
    ...
    def reset_optimizer_with_params(self, parameters):
        ...
        if isinstance(parameters, dict):
            self.optim = self.optim_cls(params_gr=OrderedSet(parameters['params_gr']), params_vs=OrderedSet(
                parameters['params_vs']), n_values=self.n_values, **self.optim_kwargs)
        else:
            self.optim = self.optim_cls(parameters, **self.optim_kwargs)

# These are helper utils for an optimization method called SOBA (https://arxiv.org/abs/2201.13409) Not used in the paper, need more testing.
# It is not applicable to our case as the grounding does not depend on the value system.
...
def nth_derivative(f, wrt, n):
    ...
    for i in range(n):
        ...
        grads = th.autograd.grad(
            f, wrt, create_graph=i < n-1, retain_graph=True)
...
def derivative21(f, wrt1, wrt2, dw2=None, retain_last=True):
    if dw2 is None:
        grads = th.autograd.grad(
            f, wrt2, create_graph=True, retain_graph=True, allow_unused=True, materialize_grads=True)
        if grads is None:
            return None, None
    else:
        grads = dw2
    f = th.sum(th.cat(tuple(g.flatten() for g in grads)))
    grads2 = th.autograd.grad(f, wrt1, retain_graph=True,
...
def gradients_soba_eff(wx, wy, vt, goal1_x, goal2_xy):
    ...
    grads_G_wrt_wx = th.autograd.grad(
        goal1_x, wx, create_graph=True, retain_graph=True, allow_unused=True, materialize_grads=True)
    grads_F_wrt_wy = th.autograd.grad(
        goal2_xy, wy, retain_graph=False, allow_unused=True, materialize_grads=True)
...
def gradients_soba(wx, wy, vt, goal1_x, goal2_xy):
    ...
    grad_G_wrt_wy, grad_G_wrt_wywx = derivative21(
        goal1_x, wx, wy, retain_last=False)

    grad_F_wrt_wx = th.autograd.grad(
        goal2_xy, wx, retain_graph=True, create_graph=True, allow_unused=True, materialize_grads=True)
    grad_F_wrt_wy = th.autograd.grad(
        goal2_xy, wy, retain_graph=False, create_graph=True, allow_unused=True, materialize_grads=True)

    if grad_G_wrt_wywx is None:
        Dlambda = grad_F_wrt_wy
    else:
        Dlambda = []
        cum_sum = sum(th.sum(gryx.mul(vt_i))
                      for gryx, vt_i in zip(grad_G_wrt_wywx, vt))
        for p in grad_F_wrt_wy:
            Dlambda.append(cum_sum + p)

    Dv = []
    cum_sum2 = sum(th.sum(grx2.mul(vt_i))
                   for grx2, vt_i in zip(grad_G_wrt_wx2, vt))
...
class BaseVSLClusterRewardLoss(preference_comparisons.RewardLoss):
    ...
    def loss_func(self, probs, target_probs, misclassified_pairs=None, apply_on_misclassified_only=False):
        ...
        if self.confident_penalty > 0.0 or self.loss_object.label_smoothing > 0.0:
            ...
            s = th.sum(self.loss_object.forward(probs_l, target_probs_l) -
                       self.confident_penalty*th.multiply(probs_l, th.log(probs_l)))
        else:
            s = th.nn.functional.binary_cross_entropy(
                probs_l, target_probs_l, reduction='sum')
        return s / len(probs)
    ...
    def forward(
        ...
        if reward_model_per_agent_id[list(reward_model_per_agent_id.keys())[0]].mode == TrainingModes.VALUE_GROUNDING_LEARNING:
            ...
            # TODO: This works because we are assuming SAME NUMBER OF EXAMPLES PER AGENT!
        ...
        for vi in range(probs_gr.shape[1]):
            # TODO: IMPORTANT: This function works because we are assuming SAME NUMBER OF EXAMPLES PER AGENT!
            nl = self.loss_func(probs_gr[:, vi], preferences_with_grounding[:, vi],
                                misclassified_pairs=missclassified_gr[vi], apply_on_misclassified_only=self.gr_apply_on_misclassified_only)
    ...
    def conciseness_penalty(self, preference_model: PreferenceModelClusteredVSL,
        ...
        for (c1, c2) in itertools.combinations(list(range(len(value_system_network_per_cluster))), 2):
            ...
            if rews_gr is None:
                ...
                assert rews_f1_in_c1_c2.shape == (len(agents_in_c1_c2)*rews_gr_per_aid[agents_in_c1_c2[0]][0].shape[0],
                                                  rews_gr_per_aid[agents_in_c1_c2[0]][0].shape[1], value_system_network_per_cluster[0].in_features), rews_f1_in_c1_c2.shape
            else:
            ...
            if rews_gr is None:
                th.testing.assert_close(rews_vs_per_agent[agents_per_cluster[c1][0]][0],
                                        rews_vs1_f1_in_c1_c2[0:rews_vs_per_agent[agents_per_cluster[c1][0]][0].shape[0]])
            ...
            if rews_vs is None:
                ...
                assert rews_vs2_f2_in_c1_c2.shape == (len(
                    agents_in_c1_c2)*rews_gr_per_aid[agents_in_c1_c2[0]][0].shape[0], rews_gr_per_aid[agents_in_c1_c2[0]][0].shape[1])

            probabilities_vs1_in_c1_c2 = preference_model.probability(
                rews_vs1_f1_in_c1_c2, rews_vs1_f2_in_c1_c2)
            probabilities_vs2_in_c1_c2 = preference_model.probability(
                rews_vs2_f1_in_c1_c2, rews_vs2_f2_in_c1_c2)

            if rews_vs is None:
                assert probabilities_vs1_in_c1_c2.shape == (
                    len(agents_in_c1_c2)*rews_gr_per_aid[agents_in_c1_c2[0]][0].shape[0],)
            # TODO minimum?? or maybe other aggregation...
            conc_penalties.append(jensen_shannon_pairwise_preferences(
                probabilities_vs1_in_c1_c2, probabilities_vs2_in_c1_c2))

            with th.no_grad():
                conciseness_real.append(discordance(probabilities_vs1_in_c1_c2, probabilities_vs2_in_c1_c2,
                                        indifference_tolerance=self.model_indifference_tolerance).detach())
        ...
        if len(conc_penalties) == 0:
            ...
            if self.conc_apply_on_worst_clusters_only:
                with th.no_grad():
                    worst_conciseness_idx_real = th.argwhere(
                        conciseness_real == th.amin(conciseness_real)).flatten()
                ...
                conc_penalty = th.sum(
                    stacked_penalties[worst_conciseness_idx_real])/len(worst_conciseness_idx_real)
            else:
                with th.no_grad():
                    # The closer to 0 the conciseness, the more important the penalty.
                    weights = (1.0 - conciseness_real)
                conc_penalty = th.dot(
                    stacked_penalties, weights)/(th.sum(weights))
...
class VSLCustomLoss(BaseVSLClusterRewardLoss):
    ...
    def forward(self, preferences, preferences_with_grounding, preference_model, reward_model_per_agent_id, fragment_idxs_per_aid,
        ...
        lossMetrics = super().forward(preferences, preferences_with_grounding, preference_model, reward_model_per_agent_id, fragment_idxs_per_aid, fragment_pairs_per_agent_id,
                                      preferences_per_agent_id, preferences_per_agent_id_with_grounding, value_system_network_per_cluster, grounding_per_value_per_cluster, agent_to_vs_cluster_assignments)
...
class ConstrainedOptimizer(VSLOptimizer):
    def __init__(self, params_gr, params_vs, n_values, lr=0.001, lr_grounding=None,
        ...
        super(ConstrainedOptimizer, self).__init__(params_gr=params_gr, params_vs=params_vs, n_values=n_values, lr=lr,
        ...
        self.set_state({'time': 0, 'lambdas': [th.tensor(
            initial_lambda, requires_grad=True) for _ in range(n_values)]})
    ...
    def set_state(self, state):
        if state is not None:
            ...
            self.time = 0
            self.lambdas = [th.tensor(
                self.initial_lambda, requires_grad=True) for vi in range(self.n_values)]
        print("LAGRANGE MULTIPLIERS AT SET STATE:", self.lambdas)
        self.optim_lambdas = th.optim.Adam(
            self.lambdas, lr=self.lr_lambda, betas=(0.5, 0.9))
        ...
        with th.no_grad():
            ...
            for vi, l in enumerate(self.lagrange_multipliers):
                ...
                if self.last_accuracy_gr[vi] < self.best_accuracies[vi]:
                    l.grad = - \
                        th.clamp(
                            (self.gr_loss_per_vi[vi] * renormalization), min=0.0).detach()
                else:
    ...
    def set_state(self, state):
        if state is not None:
            ...
            if self.use_lr_decay:
                self.optimx_scheduler = th.optim.lr_scheduler.LambdaLR(
                    lr_lambda=lambda epoch: 1/np.sqrt(self.time+1), optimizer=self.optimx)
                self.optimy_scheduler = th.optim.lr_scheduler.LambdaLR(
                    lr_lambda=lambda epoch: 1/np.sqrt(self.time+1), optimizer=self.optimy)
                self.optimv_scheduler = th.optim.lr_scheduler.LambdaLR(
                    lr_lambda=lambda epoch: 1/np.sqrt(self.time+1), optimizer=self.optimv)
        if state is None:
            self.set_state(
                {'time': 0, 'vt': [th.zeros_like(p) for p in self.params_gr]})
    ...
    def gradients(self, scalar_loss: th.Tensor, renormalization: float, fast=False) -> None:
        ...
        if fast:
            ...
            Dtheta, Dv, Dlambda = gradients_soba_eff(
                self.params_gr, self.params_vs, self.vt, self.gr_loss *
                renormalization, self.vs_loss*renormalization
            )
        else:
            Dtheta, Dv, Dlambda = gradients_soba(
                self.params_gr, self.params_vs, self.vt, self.gr_loss *
