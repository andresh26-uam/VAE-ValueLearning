...
class ClusteringRewardTrainerVSL(BasicRewardTrainerVSL):
    """Train a basic reward model."""
    def __init__(self, preference_model, loss, rng, batch_size = 32, minibatch_size = None, epochs = 1, lr = 0.001, custom_logger=None, regularizer_factory=None):
        super().__init__(preference_model, loss, rng, batch_size, minibatch_size, epochs, lr, custom_logger, regularizer_factory)
    def _train(
        self,
        ...
        with self.logger.accumulate_means("reward"):
            for epoch_num in tqdm.tqdm(range(epochs), desc="Training reward model"):
...
class CrossEntropyRewardLossForQualitativePreferences(preference_comparisons.RewardLoss):
    ...
    def forward(
        ...
        fragment_pairs: Sequence[TrajectoryPair],
        preferences: np.ndarray,
        preferences_per_value: np.ndarray,
        preference_model: preference_comparisons.PreferenceModel,
        preference_model_per_value: List[preference_comparisons.PreferenceModel],
...
class PreferenceBasedTabularMDPVSL(BaseVSLAlgorithm):
