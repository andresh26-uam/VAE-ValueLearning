...
from src.algorithms.preference_based_vsl_lib import BasicRewardTrainerVSL, ConstrainedLoss, BaseVSLClusterRewardLoss, PrefLossClasses, PreferenceModelClusteredVSL, SobaLoss, VSLCustomLoss, VSLOptimizer, calculate_accuracies, discordance, likelihood_x_is_target
...
class ClusteringRewardTrainerVSL(BasicRewardTrainerVSL):
    ...
    def __init__(self, preference_model: PreferenceModelClusteredVSL, loss: BaseVSLClusterRewardLoss, rng: np.random.RandomState, batch_size=32,
                 minibatch_size: int = None, epochs: int = 1, refining_steps_after_cluster_assignment: int = 1, initial_refining_steps: int = 10,
        ...
        """Trains a reward model for a society of agents (clusterizing groundings of values and value systems into certain number of clusters)
        ...
        Args:
            ...
            initial_refining_steps (int, optional): In the first iteration, the number of refining steps to take. Defaults to 10.
        ...
        # IF 1, 30% as validation randomly selected. If null, no validation set. k = 2,3, are not supported yet.
    ...
    def train(self, dataset: VSLPreferenceDataset, reward_model_per_agent_id: Mapping[str, AbstractVSLRewardFunction], grounding_per_value_per_cluster: List[List[th.nn.Module]], value_system_per_cluster: List[Any] = [], running_assignment_vs=None, running_assignment_gr=None, epoch_multiplier: float = 1.0, global_iteration=None, global_iterations=None, starting_assignment: ClusterAssignment = 'start_random', assignment_ranking: ClusterAssignmentMemory = None, only_eval=False) -> None:
        ...
        if self.k_mini_folds is not None:
            # TODO Think of a form of K-fold cross validation in this context...
        ...
        if self.cluster_colors_vs is None:
            self.cluster_colors_vs = assign_colors(
                len(value_system_per_cluster))
        ...
        check_grounding_value_system_networks_consistency_with_optim(
            grounding_per_value_per_cluster, value_system_per_cluster, self.optim)
        ...
        with (self.logger.accumulate_means("reward") if self.use_logger is not None else nullcontext()):
            ...
            for cvi, (mini_train_dataset, mini_val_dataset) in enumerate(cross_splits):
                ...
                self.batch_size = min(
                    len(mini_train_dataset), general_batch_size)
                ...
                if mini_val_dataset is not None:
                    ...
                    val_data_loader = self._make_data_loader(
                        mini_train_dataset)

                # recover original batch size that is used for training
                self.batch_size = general_batch_size
                ...
                for ie, epoch_num in enumerate(tqdm.tqdm(range(epochs), desc="Training reward model")):
                    with (self.logger.add_key_prefix(f"ep-{epoch_num}-cv-{cvi}") if self.use_logger is not None else nullcontext()):
                        ...
                        for fragment_pairs, preferences, preferences_per_grounding, agent_ids in dataloader:
                            ...
                            check_grounding_value_system_networks_consistency_with_optim(
                                grounding_per_value_per_cluster, value_system_per_cluster, self.optim)
                            check_assignment_consistency(grounding_per_value_per_cluster, value_system_per_cluster, assignment_aid_to_gr_cluster=running_assignment_gr,
                                                         assignment_aid_to_vs_cluster=running_assignment_vs, reward_models_per_aid=reward_model_per_agent_id)
                            if not only_eval:
                            ...
                            for r in range(training_steps):
                                ...
                                self.loss.gradients(
                                    scalar_loss=loss, renormalization=1.0)
                ...
                with th.no_grad():
                    ...
                    if hasattr(self.optim, 'get_state'):
                        ...
                        new_assignment.optimizer_state = self.optim.get_state(
                            copy=True)
                    ...
                    val_fragment_pairs, val_preferences, val_preferences_per_grounding, val_agent_ids = map(
                        lambda x: np.concatenate(
                            x, axis=0) if len(data) > 1 else x[0],
                        zip(*data)
                    )
    ...
    def gr_discordances(self, grounding_per_value_per_cluster, gr_cluster_to_agents_assignment, fragment_pairs_per_aid, preferences_grounding_per_aid, probs_gr_per_agent):
        ...
        agents_assigned_to_each_cluster_pair = [{(c1, c2): (gr_cluster_to_agents_assignment[vi][c1], gr_cluster_to_agents_assignment[vi][c2])
                                                 for c1, c2 in itertools.combinations(range(len(gr_cluster_to_agents_assignment[vi])), r=2)} for vi in range(self.n_values)]
        gr_discordance_of_each_agent = {vi: {} for vi in range(self.n_values)}
        gr_inter_dist_for_each_pair_of_clusters = {
            vi: {} for vi in range(self.n_values)}

        for vi in range(self.n_values):
            if agents_assigned_to_each_cluster_pair[vi] == {}:
                agents_assigned_to_each_cluster_pair[vi] = {
                    (0, 0): (list(fragment_pairs_per_aid.keys()), [])}
        ...
        gt_is_numpy = isinstance(preferences_grounding_per_aid[list(
            preferences_grounding_per_aid.keys())[0]][0], np.ndarray)
        for vi in range(self.n_values):
            for (c1, c2), (assigned_c1, assigned_c2) in agents_assigned_to_each_cluster_pair[vi].items():
                ...
                for (ci, assigned_ci) in [(c1, assigned_c1), (c2, assigned_c2)]:
                    if ci not in fragments_on_each_cluster[vi].keys():
                        fragments_on_each_cluster[vi][ci] = np.array(
                            [pair for aid, fragaids in fragment_pairs_per_aid.items() if (aid in assigned_ci) for pair in fragaids])
                        preferences_on_each_cluster[vi][ci] = np.array(
                            [pr for aid, praids in preferences_grounding_per_aid.items() if (aid in assigned_ci) for pr in praids[:, vi]])
                        ...
                        for aid in assigned_ci:
                            ...
                            gr_intra_cluster_distances[vi].append(
                                representativity_error_on_vi)
                            gr_discordance_of_each_agent[vi][aid] = representativity_error_on_vi
                            assert len(gr_intra_cluster_distances[vi]) <= len(
                                preferences_grounding_per_aid.keys())
                if len(assigned_c1) > 0 and len(assigned_c2) > 0:
                    fragments_on_v1c1c2 = [
                        *fragments_on_each_cluster[vi][c1], *fragments_on_each_cluster[vi][c2]]
                    acc_div = self._preference_model.conciseness_pairwise_gr(fragments=fragments_on_v1c1c2,
                                                                             grounding1=grounding_per_value_per_cluster[
                                                                                 vi][c1],
                                                                             grounding2=grounding_per_value_per_cluster[
                                                                                 vi][c2],
                                                                             value_idx=vi, indifference_tolerance=self.loss.model_indifference_tolerance
                                                                             )

                    gr_inter_cluster_distances[vi].append(acc_div)
                    gr_inter_dist_for_each_pair_of_clusters[vi][(
                        c1, c2)] = acc_div
                    print(
                        f"VALUE {vi}, Clusters {c1}, {c2}. Inter cluster distance: {acc_div}")
                else:
                    if len(assigned_c1) == 0:
                        print(
                            f"VALUE {vi}, Cluster {c1} is of length 0, skipping")
                    else:
                        print(
                            f"VALUE {vi}, Cluster {c2} is of length 0, skipping")
    ...
    def vs_discordances(self, reward_model_per_agent_id, value_system_per_cluster, vs_cluster_to_agents_assignment, fragment_pairs_per_aid, preferences_per_aid, probs_per_aid, assume_1_grounding=True, rews_gr_per_aid: Dict[str, Tuple[th.Tensor, th.Tensor]] = None,
        ...
        fragments_on_each_cluster_vs = {(c1, c2): (vs_cluster_to_agents_assignment[c1], vs_cluster_to_agents_assignment[c2]) for c1, c2 in itertools.combinations(
            range(len(vs_cluster_to_agents_assignment)), r=2)}
        if len(fragments_on_each_cluster_vs) == 0:
            fragments_on_each_cluster_vs = {(0, 0): (
                vs_cluster_to_agents_assignment[0], vs_cluster_to_agents_assignment[0])}
        ...
        for (c1, c2), (assigned_c1, assigned_c2) in fragments_on_each_cluster_vs.items():
            for (ci, assigned_ci) in [(c1, assigned_c1), (c2, assigned_c2)]:
                if ci not in fragments_on_each_cluster_per_aid.keys():
                    ...
                    if not assume_1_grounding:  # TODO
                        fragments_on_each_cluster_per_aid[ci] = {
                            aid: fragment_pairs_per_aid[aid] for aid in assigned_ci}
                        preferences_on_each_cluster_per_aid[ci] = {
                            aid: preferences_per_aid[aid] for aid in assigned_ci}
                    ...
                    for aid in assigned_ci:
                        ...
                        vs_intra_cluster_distances.append(
                            representativity_error)
            ...
            if len(assigned_c1) > 0 and len(assigned_c2) > 0:
                ...
                if not assume_1_grounding:
                    fragments_on_each_cluster_c1c2 = fragments_on_each_cluster_per_aid[
                        c1] | fragments_on_each_cluster_per_aid[c2]
                    ...
                    with th.no_grad():
                        ...
                        if rews_vs is None:
                            rews_f1_in_c1_c2 = th.cat(
                                [rews_gr_per_aid[aid][0] for aid in agents_in_c1_c2], dim=0)
                            rews_f2_in_c1_c2 = th.cat(
                                [rews_gr_per_aid[aid][1] for aid in agents_in_c1_c2], dim=0)
                        else:
                            rews_f1_in_c1_c2 = rews_gr[0]
                            rews_f2_in_c1_c2 = rews_gr[1]
                        rews_vs1_f1_in_c1_c2 = vs1.forward(
                            rews_f1_in_c1_c2).squeeze(-1)
                        ...
                        rews_vs1_f2_in_c1_c2 = vs1.forward(
                            rews_f2_in_c1_c2).squeeze(-1)

                        rews_vs2_f1_in_c1_c2 = vs2.forward(
                            rews_f1_in_c1_c2).squeeze(-1)
                        rews_vs2_f2_in_c1_c2 = vs2.forward(
                            rews_f2_in_c1_c2).squeeze(-1)

                        probabilities_vs1_in_c1_c2 = self._preference_model.probability(
                            rews_vs1_f1_in_c1_c2, rews_vs1_f2_in_c1_c2)
                        probabilities_vs2_in_c1_c2 = self._preference_model.probability(
                            rews_vs2_f1_in_c1_c2, rews_vs2_f2_in_c1_c2)
                        ql_div_vs = discordance(
                            probs=probabilities_vs1_in_c1_c2, gt_probs=probabilities_vs2_in_c1_c2, indifference_tolerance=0.0)

                vs_inter_cluster_distances.append(ql_div_vs)
                vs_inter_dist_for_each_pair_of_clusters[(c1, c2)] = ql_div_vs
                print(
                    f"VS Clusters {c1} {c2}. Inter cluster distances: Qualitative {ql_div_vs}")
            else:
                if len(assigned_c1) == 0:
        ...
        if self.debug_mode:
            ...
            for aid in reward_model_per_agent_id.keys():
                ...
                if aid in agent_to_vs_cluster_assignments.keys():
                    ...
                    networks_to_train.append(
                        value_system_network_per_cluster[cvs_])
            optim_params = []
    ...
    def cluster_assignment(self, fragment_pairs, preferences, preferences_with_grounding,
        ...
        using_assignment = isinstance(
            use_assignment, ClusterAssignment) and use_assignment.agent_to_gr_cluster_assignments != []
        if using_assignment:
            self.update_training_networks_from_assignment(reward_model_per_agent_id, grounding_per_value_per_cluster, value_system_network_per_cluster,
                                                          use_assignment, prev_agent_to_gr=running_assignment_gr, prev_agent_to_vs=running_assignment_vs)
            ...
            vs_cluster_to_agents_assignment = [
                list() for _ in value_system_network_per_cluster]
            ...
            for aid, rew_aid in reward_model_per_agent_id.items():
                if not using_assignment:
                    ...
                    aid_likelihood_per_vs_cluster = [
                        1.0]*len(value_system_network_per_cluster)
                ...
                if not using_assignment:
                    agent_fragments_idxs_temp_dict = {
                        aid: list(range(len(agent_fragments)))}
                    fragments_aid_temp_dict = {aid: agent_fragments}

                for vi, vi_profile in enumerate(self.basic_profiles):
                    # This code also supports clusterizing value groundings. Not explored yet.
                    if not using_assignment:
                        for icluster, network_vi_c in enumerate(grounding_per_value_per_cluster[vi]):
                            ...
                            if len(grounding_per_value_per_cluster[vi]) > 1:
                                ...
                                rew_aid.set_mode(
                                    TrainingModes.VALUE_GROUNDING_LEARNING)
                                probs_vs, _ = self._preference_model.forward(
                                    fragments_aid_temp_dict, custom_model_per_agent_id=rew_aid, fragment_pairs_idxs_per_agent_id=agent_fragments_idxs_temp_dict, only_for_alignment_function=vi_profile)
                                gt_probs = agent_preferences_with_grounding[:, vi]
                                aid_likelihood_per_value_per_cluster[vi][icluster] = likelihood_x_is_target(probs_vs.detach(), gt_probs.detach(
                                ), mode='th', slope=0.3, adaptive_slope=False, qualitative_mode=self.qualitative_cluster_assignment, indifference_tolerance=self.loss.model_indifference_tolerance)
                        ...
                        gr_cluster_to_agents_assignment[vi][assigned_cluster].append(
                            aid)
                        ...
                        if len(grounding_per_value_per_cluster[vi]) > 1 and self.debug_mode:
                            ...
                            for i, value in enumerate(aid_likelihood_per_value_per_cluster[vi]):
                                ...
                                print(
                                    f"{color}{value}{Style.RESET_ALL}", end=', ')
                            print(
                ...
                if not using_assignment:
                    ...
                    for ivscluster, network_vs_c in enumerate(value_system_network_per_cluster):
                        ...
                        rew_aid.set_trained_alignment_function_network(
                            network_vs_c)
                        rew_aid.set_mode(self.train_mode)
                        probs_vs, _ = self._preference_model.forward(fragments_aid_temp_dict, custom_model_per_agent_id=rew_aid,
                                                                     fragment_pairs_idxs_per_agent_id=agent_fragments_idxs_temp_dict, only_for_alignment_function=rew_aid.get_learned_align_function())
                        ...
                        aid_likelihood_per_vs_cluster[ivscluster] = likelihood_x_is_target(probs_vs.detach(), gt_probs.detach(
                        ), mode='th', slope=0.3, adaptive_slope=False, qualitative_mode=self.qualitative_cluster_assignment, indifference_tolerance=self.loss.model_indifference_tolerance)
                    ...
                    rew_aid.set_trained_alignment_function_network(
                        value_system_network_per_cluster[assigned_cluster_vs])
                    ...
                    vs_cluster_to_agents_assignment[assigned_cluster_vs].append(
                        aid)
                    if len(value_system_network_per_cluster) > 1 and self.debug_mode:
                        ...
                        print(
                            f"), {self.cluster_colors_vs[assigned_cluster_vs]} {rew_aid.get_learned_align_function()} - VS Cluster {assigned_cluster_vs}: {aid_likelihood_per_vs_cluster[assigned_cluster_vs]}, {Style.RESET_ALL}")
        ...
        if not using_assignment:
            gr_cluster_to_agents_assignment = convert_nested_list_to_tuple(
                gr_cluster_to_agents_assignment)
            vs_cluster_to_agents_assignment = convert_nested_list_to_tuple(
                vs_cluster_to_agents_assignment)
            ...
            if self.debug_mode:
                ...
                check_assignment_consistency(assignment.grounding_per_value_per_cluster, value_system_network_per_cluster, running_assignment_gr, running_assignment_vs, assignment.reward_model_per_agent_id
                                             )
    ...
    def update_training_networks_from_assignment(self, reward_model_per_agent_id: Dict[str, AbstractVSLRewardFunction], grounding_per_value_per_cluster, value_system_per_cluster, reference_assignment: ClusterAssignment, prev_agent_to_vs=None, prev_agent_to_gr=None):
        ...
        with th.no_grad():
            ...
            for aid, model in reward_model_per_agent_id.items():
                ...
                if self.debug_mode:
                    th.testing.assert_close(reference_model.get_trained_alignment_function_network().state_dict(
                    ), reference_assignment.value_system_per_cluster[reference_assignment.agent_to_vs_cluster_assignments[aid]].state_dict())
                ...
                for vi in range(self.n_values):
                    ...
                    grounding_per_value_per_cluster[vi][reference_assignment.agent_to_gr_cluster_assignments[aid][vi]].load_state_dict(
                        deepcopy(reference_model.get_network_for_value(vi).state_dict()))
                    # model.get_network_for_value(vi).load_state_dict(reference_model.get_network_for_value(vi).state_dict())
                    model.set_network_for_value(
                        vi, grounding_per_value_per_cluster[vi][reference_assignment.agent_to_gr_cluster_assignments[aid][vi]])
                    if self.debug_mode:
                        th.testing.assert_close(model.get_network_for_value(
                            vi).state_dict(), reference_model.get_network_for_value(vi).state_dict())
                        th.testing.assert_close(model.get_network_for_value(vi).state_dict(
                        ), grounding_per_value_per_cluster[vi][prev_agent_to_gr[aid][vi]].state_dict())
                ...
                if self.debug_mode:
                    ...
                    ).state_dict(), value_system_per_cluster[prev_agent_to_vs[aid]].state_dict())
                value_system_per_cluster[reference_assignment.agent_to_vs_cluster_assignments[aid]].load_state_dict(
                    deepcopy(reference_model.get_trained_alignment_function_network().state_dict()))
                model.set_trained_alignment_function_network(
                    value_system_per_cluster[reference_assignment.agent_to_vs_cluster_assignments[aid]])
                prev_agent_to_vs[aid] = reference_assignment.agent_to_vs_cluster_assignments[aid]
                if self.debug_mode:
                    th.testing.assert_close(reference_model.get_trained_alignment_function_network().state_dict(
                    ), value_system_per_cluster[reference_assignment.agent_to_vs_cluster_assignments[aid]].state_dict())

                    th.testing.assert_close(model.get_trained_alignment_function_network().state_dict(
                    ), value_system_per_cluster[reference_assignment.agent_to_vs_cluster_assignments[aid]].state_dict())

                    th.testing.assert_close(model.get_trained_alignment_function_network().state_dict(
                    ), reference_model.get_trained_alignment_function_network().state_dict())

                    th.testing.assert_close(model.get_trained_alignment_function_network().state_dict(
                    ), reference_model.get_trained_alignment_function_network().state_dict())
                    np.testing.assert_almost_equal(model.get_learned_align_function(
                    ), reference_model.get_learned_align_function())
            if self.debug_mode:
                for aid, model in reward_model_per_agent_id.items():
                    reference_model = reference_assignment.reward_model_per_agent_id[aid]
                    vsNetwork: ConvexAlignmentLayer = value_system_per_cluster[
                        reference_assignment.agent_to_vs_cluster_assignments[aid]]
                    th.testing.assert_close(reference_model.get_trained_alignment_function_network(
                    ).state_dict(), vsNetwork.state_dict())

                    np.testing.assert_allclose(model.get_learned_align_function(
                    ), reference_assignment.reward_model_per_agent_id[aid].get_learned_align_function())
        ...
        if isinstance(self.loss, VSLCustomLoss):
            ...
            if self.debug_mode:
                should_be_wx = {param for rew in reward_model_per_agent_id.values() for network in [
                    rew.get_network_for_value(i) for i in range(self.n_values)] for param in network.parameters()}
                should_be_wy = {param for rew in reward_model_per_agent_id.values(
                ) for param in rew.get_trained_alignment_function_network().parameters()}
                ...
                assert self.optim.params_gr == {param for vi, cluster in enumerate(
                    grounding_per_value_per_cluster) for network in cluster for param in network.parameters()}, "Mismatch in wx parameters"
            ...
            self.loss.set_parameters(params_gr=self.optim.params_gr,
                                     params_vs=self.optim.params_vs, optim_state=self.optim.get_state())
        if self.debug_mode:
            updated_params = {
                param for group in self.optim.param_groups for param in group['params']
            }
...
class PreferenceComparisonVSL(preference_comparisons.PreferenceComparisons):
    ...
    def train(
        ...
        best_assignments_list = ClusterAssignmentMemory(
            max_size=max_assignment_memory, n_values=len(grounding_per_value_per_cluster))

        self.training_networks = {'rw': reward_model_per_agent_id,
                                  "gr": grounding_per_value_per_cluster, "vc": value_system_per_cluster, 'version': 0}
        ...
        for global_iter, num_pairs in enumerate(schedule):
            ...
            if global_iter == 0:
                ...
                while len(best_assignments_list) < best_assignments_list.max_size:
                    reference_assignment = self.generate_random_assignment(
                        reward_model_per_agent_id, grounding_per_value_per_cluster, value_system_per_cluster, original_agent_to_gr_cluster_assignments, original_agent_to_vs_cluster_assignments)
                    self.reward_trainer.update_training_networks_from_assignment(reward_model_per_agent_id, grounding_per_value_per_cluster, value_system_per_cluster,
                                                                                 reference_assignment, prev_agent_to_gr=original_agent_to_gr_cluster_assignments, prev_agent_to_vs=original_agent_to_vs_cluster_assignments)
            ...
            if (random.random() < exploration) or use_random:
                ...
                value_system_deviation = 2.0 - \
                    starting_assignment.representativity_vs(
                        aggr=np.min)-starting_assignment.conciseness_vs()
                ...
                reference_assignment.optimizer_state = deepcopy(
                    starting_assignment.optimizer_state)
                reference_assignment.explored = False
                self.reward_trainer.update_training_networks_from_assignment(reward_model_per_agent_id, grounding_per_value_per_cluster, value_system_per_cluster,
                                                                             reference_assignment, prev_agent_to_gr=original_agent_to_gr_cluster_assignments, prev_agent_to_vs=original_agent_to_vs_cluster_assignments)
                starting_assignment = reference_assignment
            else:
                starting_assignment.explored = True
                self.reward_trainer.update_training_networks_from_assignment(reward_model_per_agent_id, grounding_per_value_per_cluster, value_system_per_cluster,
                                                                             starting_assignment, prev_agent_to_gr=original_agent_to_gr_cluster_assignments, prev_agent_to_vs=original_agent_to_vs_cluster_assignments)
            ...
            exploration = self.reward_trainer.initial_exploration_rate * \
                ((num_iterations-global_iter)/num_iterations)  # TODO epsilon decay?
            print(f"ITERATION {global_iter}/{num_iterations}.",
            ...
            best_assignment: ClusterAssignment = best_assignments_list.get_best_assignment(
                consider_only_unexplored=False, lexicographic_vs_first=True)
            # Save the best assignment of each iteration
            best_assignment.save(
                historical_assignments_save_folder, f"best_assignment_iter_{global_iter}.pkl")
            ...
            with (self.logger.accumulate_means("agent") if self.use_logger is not None else nullcontext()):
                if self.use_logger:
                    self.logger.log(
                        f"Training agent for {num_steps} timesteps")
        ...
        best_assignment: ClusterAssignment = best_assignments_list.get_best_assignment(
            consider_only_unexplored=False, lexicographic_vs_first=False)
        self.reward_trainer.update_training_networks_from_assignment(reward_model_per_agent_id, grounding_per_value_per_cluster, value_system_per_cluster,
                                                                     prev_agent_to_gr=original_agent_to_gr_cluster_assignments, prev_agent_to_vs=original_agent_to_vs_cluster_assignments, reference_assignment=best_assignment)
    ...
    def generate_mutated_assignment(self, reward_model_per_agent_id, grounding_per_value_per_cluster, value_system_per_cluster, assignment_gr, assignment_vs,
        ...
        with th.no_grad():
            grounding_per_value_per_cluster_c = [[None for c in range(
                len(cs_per_vi))] for cs_per_vi in grounding_per_value_per_cluster]
            value_system_per_cluster_c = None
            if value_system_per_cluster is not None:
                value_system_per_cluster_c = [
                    None for _ in range(len(value_system_per_cluster))]
            random_reward_model: Dict[str, AbstractVSLRewardFunction] = {}

            agent_to_gr_cluster_assignments = {}
            agent_to_vs_cluster_assignments = {}

            assignment_gr_new = [[list() for _ in range(len(grounding_per_value_per_cluster_c[vi_]))]
                                 for vi_ in range(len(grounding_per_value_per_cluster_c))]
            assignment_vs_new = [list()
                                 for _ in range(len(value_system_per_cluster_c))]

            grounding_per_value_per_cluster_c = deepcopy(
                grounding_per_value_per_cluster)
            value_system_per_cluster_c = deepcopy(value_system_per_cluster)

            l_prev = len([i for i in range(len(prev_cs_to_agent))
                         if len(prev_cs_to_agent[i]) > 0])
            expand_or_reduce = random.random() < 0.5

            if len(value_system_per_cluster) > 1:
                valid_new_clusters = [i for i in range(len(prev_cs_to_agent)) if len(
                    prev_cs_to_agent[i]) > 0]  # The old ones
            ...
            for aid, cluster_vs_aid in assignment_vs.items():
                ...
                for vi, cluster_vi_aid in grclusterby_value_aid.items():
                    ...
                    if len(assignment_gr_new[vi][cluster_vi_aid]) == 1:
                        for param in grounding_per_value_per_cluster_c[vi][cluster_vi_aid].parameters():
                            if param.requires_grad:
                                ...
                                normal = th.empty_like(param).normal_(
                                    0, mutation_scale*grounding_deviation*th.norm(param))
                    ...
                    rc.set_network_for_value(
                        vi, grounding_per_value_per_cluster_c[vi][cluster_vi_aid])
                ...
                if value_system_per_cluster_c is not None and len(assignment_vs_new[cluster_vs_aid]) == 1:
                    ...
                    for param in value_system_per_cluster_c[cluster_vs_aid].parameters():
                        if param.requires_grad:
                            param: th.Tensor
                            normal = th.empty_like(param).normal_(
                                0, mutation_scale*value_system_deviation*th.norm(param))
                            param.add_(normal)

                rc.set_trained_alignment_function_network(
                    value_system_per_cluster_c[cluster_vs_aid])
                random_reward_model[aid] = rc

            assert set(random_reward_model.keys()) == set(
                reward_model_per_agent_id.keys())
    ...
    def generate_random_assignment(self, reward_model_per_agent_id, grounding_per_value_per_cluster, value_system_per_cluster, assignment_gr, assignment_vs):
        ...
        for vi, cs_per_vi in enumerate(grounding_per_value_per_cluster):
            for cs in range(len(cs_per_vi)):
                ...
                rc.reset_learned_grounding_function()
                grounding_per_value_per_cluster_c[vi][cs] = rc.get_network_for_value(
                    vi)
        value_system_per_cluster_c = None
        if value_system_per_cluster is not None:
            value_system_per_cluster_c = []
        ...
        agent_to_gr_cluster_assignments = {}
        agent_to_vs_cluster_assignments = {}
        assignment_gr = [[list() for _ in range(len(grounding_per_value_per_cluster_c[vi]))]
                         for vi_ in range(len(grounding_per_value_per_cluster_c))]
        assignment_vs = [list()
                         for _ in range(len(value_system_per_cluster_c))]
        for aid in self.complete_dataset.agent_data.keys():
            ...
            for vi in range(example_model.values_net.num_outputs):
                cs = np.random.choice(
                    len(grounding_per_value_per_cluster_c[vi]))
...
def load_historic_assignments(experiment_name, sample=20):
    ...
    if not os.path.exists(save_folder):
        ...
        matching_folders = [folder for folder in os.listdir(
            parent_folder) if folder.startswith(experiment_name)]
        if matching_folders:
            save_folder = os.path.join(parent_folder, matching_folders[0])
        else:
            raise FileNotFoundError(
                f"Historic assignments directory not found: {save_folder}")
...
class PreferenceBasedClusteringTabularMDPVSL(BaseVSLAlgorithm):
    ...
    def train(self, max_iter=5000, mode=TrainingModes.VALUE_GROUNDING_LEARNING, assumed_grounding=None,
        ...
        self.historical_assignments_save_folder = os.path.join(
            ASSIGNMENT_CHECKPOINTS, experiment_name)
        ...
        historic_assignments, env_state, _ = load_historic_assignments(
            experiment_name)
    ...
    def train_simultaneous_vsl(self, max_iter, n_seeds_for_sampled_trajectories, n_sampled_trajs_per_seed):
        ...
        check_assignment_consistency(grounding_per_value_per_cluster, value_system_network_per_cluster,
                                     assignment_aid_to_gr_cluster, assignment_aid_to_vs_cluster, reward_models_per_aid)
        ...
        assert self.training_reward_nets_per_agent.keys(
        ) == self.training_reward_nets_per_agent.keys()
        for aid in self.training_reward_nets_per_agent.keys():
            th.testing.assert_close(self.training_reward_nets_per_agent[aid].state_dict(), assignment_memory.get_best_assignment(
                consider_only_unexplored=False, lexicographic_vs_first=False).reward_model_per_agent_id[aid].state_dict())
    ...
    def _train_global(self, max_iter, optim_cls, optim_kwargs, parameters: Dict[str, List[th.nn.Parameter]],
        ...
        check_assignment_consistency(grounding_per_value_per_cluster, value_system_per_cluster,
                                     assignment_aid_to_gr_cluster, assignment_aid_to_vs_cluster, reward_nets_per_agent)
    ...
    def evaluate_assignment(self, assignment: ClusterAssignment, dataset: VSLPreferenceDataset):
        ...
        dataloader = self.pref_comparisons.reward_trainer._make_data_loader(
            dataset)
