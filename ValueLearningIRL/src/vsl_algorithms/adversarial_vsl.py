...
from src.vsl_policies import VAlignedDiscreteSpaceActionPolicy, ValueSystemLearningPolicy, VAlignedDictDiscreteStateActionPolicyTabularMDP
...
class AIRLforVSL(common.AdversarialTrainer):
    ...
    def _get_log_policy_act_prob(
        ...
        obs_th: th.Tensor,
        acts_th: th.Tensor,
        states_th: th.Tensor = None # obs_th should already be the state in POMDP tabular environments
    ) -> Optional[th.Tensor]:
        ret = None
    ...
    def reward_function_for_policy_training(self, obs: np.ndarray, action: np.ndarray, next_obs: np.ndarray, dones: np.ndarray, ground_truth=False):
        with th.no_grad():
            obs = th.as_tensor(obs, dtype=self.advsl.current_net.dtype, device=self.advsl.current_net.device).long()
            action = th.as_tensor(action, dtype=self.advsl.current_net.dtype, device=self.advsl.current_net.device).long()
            next_obs = th.as_tensor(next_obs, dtype=self.advsl.current_net.dtype, device=self.advsl.current_net.device).long()
            # TODO: possibly add support for the dones attribute...
            reward_real = self.advsl.env.reward_matrix_per_align_func(self.advsl.current_target)[obs, action]
...
class AdversarialVSL(BaseVSLAlgorithm):
    ...
    def __init__(
        ...
        *,
        ...
        self.learned_policy_per_va = LearnerValueSystemLearningPolicy(env=self.env, learner_class=self.learner_class, masked=self.masked_policy, policy_class=self.policy_class, learner_kwargs=self.learner_kwargs, policy_kwargs=self.policy_kwargs, state_encoder=lambda obs, info: obs)
        self.expert_learned_policy_per_va = LearnerValueSystemLearningPolicy(env=self.env, learner_class=self.learner_class, masked=self.masked_policy, policy_class=self.policy_class, learner_kwargs=self.learner_kwargs, policy_kwargs=self.policy_kwargs, state_encoder=lambda obs, info: obs)
    ...
    def train_vsl(self, max_iter, n_seeds_for_sampled_trajectories, n_sampled_trajs_per_seed, target_align_func) -> Dict[Any, AbstractVSLRewardFunction]:
        ...
        self.current_net.set_mode(mode=TrainingModes.VALUE_SYSTEM_IDENTIFICATION)
        ...
        demos = self.vsi_sampler([target_align_func], n_seeds_for_sampled_trajectories, n_sampled_trajs_per_seed)
        print(len(demos), n_sampled_trajs_per_seed, n_seeds_for_sampled_trajectories)
        
        learning_algo : PPO = self.learned_policy_per_va.get_learner_for_alignment_function(target_align_func)
        expert_algo : PPO = self.expert_learned_policy_per_va.get_learner_for_alignment_function(target_align_func)
        ...
        airl_trainer.train(max_iter, callback=lambda t: (airl_trainer.logger.record("mean/Target",target_align_func),
                                                       airl_trainer.logger.record("mean/Learned", tuple([float(f"{a:0.5f}") for a in self.current_net.get_learned_align_function()]) ),
                                                       airl_trainer.logger.record("mean/Learned Bias",self.current_net.get_learned_profile(with_bias=True)[1]),
                                                       airl_trainer.logger.record("mean/Learned OTHER??",list(self.current_net.get_learned_grounding().parameters())),
        ...
        print("TRAJS BEFORE", [t.obs for t in trajs_before[0:5] if t.obs[0] in [tt.obs[0] for tt in demos]])
        print("TRAJS LEARN" , [t.obs for t in trajs[0:5] if t.obs[0] in [tt.obs[0] for tt in demos]])
        print("TRAJS REAL", [t.obs for t in expert_trajs[0:5] if t.obs[0] in [tt.obs[0] for tt in demos]])
        ...
        print("mean reward stochastic after training:", np.mean(learner_rewards_after_training_st))
        print("mean reward stochastic before training:", np.mean(learner_rewards_before_training_st))
...
class TabularAdversarialVSL(BaseTabularMDPVSLAlgorithm, AdversarialVSL):
    ...
    def train_vsl(self, max_iter, n_seeds_for_sampled_trajectories, n_sampled_trajs_per_seed, target_align_func):
        ...
        demos = self.vsi_sampler([target_align_func], n_seeds_for_sampled_trajectories*10, n_sampled_trajs_per_seed)
        
        print(len(demos), n_seeds_for_sampled_trajectories, n_sampled_trajs_per_seed)
        ...
        airl_trainer = TabularAIRLforVSL(
            demonstrations=demos,
            demo_batch_size=1000, # demo batch size is not used, it is number of trajs to sample and this.
        ...
        airl_trainer.train(1000000, callback=lambda t: (airl_trainer.logger.record("mean/Target",target_align_func),airl_trainer.logger.record("mean/Learned", tuple([float(f"{a:0.5f}") for a in self.current_net.get_learned_align_function()]))))  # Train for 2_000_000 steps to match expert.
...
class TabularAIRLforVSL(AIRLforVSL):
    ...
    def __init__(self, *, demonstrations, demo_batch_size, sampled_trajs_per_round, venv, reward_net=None, adversarialVSL: TabularAdversarialVSL =None, alignment=None, stochastic_gen=True, **kwargs):
        dummy_algo = adversarialVSL.learner_class(policy=adversarialVSL.policy_class, env=venv, policy_kwargs=adversarialVSL.policy_kwargs, **adversarialVSL.learner_kwargs)
        super().__init__(demonstrations=demonstrations, demo_batch_size=demo_batch_size, venv=venv, gen_algo=dummy_algo, expert_algo=dummy_algo, reward_net=reward_net, adversarialVSL=adversarialVSL, alignment=alignment, stochastic_gen=stochastic_gen, **kwargs)
    ...
    def train_gen(
        ...
        with self.logger.accumulate_means("gen"):
            ...
            self.advsl.learned_policy_per_va.set_policy_for_va(self.advsl.current_target, policy_matrix)
            self._global_step += 1
        ...
        *,
        ...
        with self.logger.accumulate_means("disc"):
            ...
            for batch in batch_iter:
                ...
                exp_visitation_counts, gen_visitation_counts = self.calculate_visitation_counts(batch)
                
                labels = 1/2.0 + ((exp_visitation_counts - gen_visitation_counts)/(exp_visitation_counts + gen_visitation_counts + 1e-16)).float()/2.0
                ...
                loss = F.binary_cross_entropy_with_logits(
                    disc_logits,
                    batch["labels_expert_is_one"].float(), #TODO ? the variable "labels" should work better?
                )
    ...
    def calculate_visitation_counts(self, batch):
        ...
        gen_visitation_matrix = th.zeros((self.advsl.env.state_dim,), dtype=th.long)
        gen_visitation_matrix[unique_vals] = gen_counts
        exp_visitation_matrix = th.zeros_like(gen_visitation_matrix, dtype=th.long)
    ...
    def _next_expert_batch(self):
        ...
        expert_trajs = self.advsl.vsi_sampler([self.advsl.current_target], self.sampled_trajs_per_round, 5)
        ...
        if len(expert_all['obs']) < self.demo_batch_size:
            ...
            ind = np.random.randint(len(expert_all['obs']), size=self.demo_batch_size) 
    ...
    def _next_gen_batch(self, expert_initial_states = None):
        if expert_initial_states is not None:
            initial_states_of_expert = expert_initial_states #np.array(expert_batch['obs']).flatten()
        ...
        gen_samples = rollout.flatten_trajectories_with_rew(trajectories_from_initial_states)
        ...
        for r in tqdm.tqdm(range(0, n_rounds), desc="round"):
            ...
            for _ in range(self.n_disc_updates_per_round):
                with networks.training(self.reward_train):
                    ...
                    self.train_disc(expert_samples=expert_samples,gen_samples=gen_samples)
            if callback:
                callback(r)
    ...
    def _get_log_policy_act_prob(self, obs_th, acts_th):
        ...
        log_policy_act_prob_th = th.log(th.clamp(policy_acts_probs[obs_th, acts_th], 0.001, 0.999))
