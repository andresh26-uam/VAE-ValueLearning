...
from src.vsl_reward_functions import TrainingModes, LinearVSLRewardFunction, cost_model_from_reward_net, squeeze_r
...
def get_demo_oms_from_trajectories(trajs: Iterable[types.Trajectory], state_dim, discount=1, reference_experts: Dict[tuple, List[types.Trajectory]] = None, groupby='odpr') -> dict:
    ...
    for traj in trajs:
        ...
        # if groupby == 'odpr' else (traj.infos[0]['orig'], traj.infos[0]['des'])
        orig_des_profile = (
            (traj.infos[0]['orig'], traj.infos[0]['des']), traj.infos[0]['profile'])
...
class TrainingSetModes(enum.Enum):
    #  Used for profile learning of a society sampling trajectories according to a probabilty distribution of profiles.
    PROFILED_SOCIETY = 'profiled_society'
...
class MCEIRL_RoadNetwork(base.DemonstrationAlgorithm[types.TransitionsMinimal]):
    ...
    def set_reward_net(self, reward_net: LinearVSLRewardFunction):

        self._reward_net = reward_net
        self.optimizer = self.optimizer_cls(
            self._reward_net.parameters(), **self.optimizer_kwargs)
    ...
    def __init__(
        ...
        *,
        ...
        # list of expert trajectories just to see different origin destinations and profiles
        ...
        if training_profiles == 'auto':
            self.training_profiles = list(
                set(tuple(t.infos[0]['profile']) for t in self.expert_trajectories))
        else:
        ...
        self.expert_policy: SimplePolicy = expert_policy if isinstance(
            expert_policy, SimplePolicy) else SimplePolicy.from_policy_matrix(expert_policy, self.env)
        ...
        if self.use_mce_om:  # TODO: UNUSED ?
            ...
            for pr in self.training_profiles:
                ...
                _, demo_state_om = self.mce_occupancy_measures(
                    pi=expert_policy[pr], profile=pr, od_list=self.env.od_list_int)
                for odpr, om_dif in demo_state_om.items():
                    self.demo_state_om_per_profile[odpr] = om_dif
        else:

            sampled_expert_trajectories = self.expert_policy.sample_trajectories(
                stochastic=self.stochastic_expert, repeat_per_od=self.n_repeat_per_od_monte_carlo, with_profiles=self.training_profiles)
        ...
        self.optimizer = optimizer_cls(
            reward_net.parameters(), **optimizer_kwargs)
        ...
        self.default_vi_policy = ValueIterationPolicyRoadWorld(
            self.env)  # Starts with random policy
        ...
        self._policy = TabularPolicyPerProfile(
            ...
            pi={
                pr: self.vi_policy_per_profile[pr].pi_from_d for pr in self.training_profiles},
            rng=self.rng,
        )
    ...
    def mce_occupancy_measures(
        ...
        *,
        ...
        if pi is None:
            pi = self.mce_partition_fh(
                reward_matrix=reward, profile=profile, od_list=od_list_int)
        Dcums = {}
        Ds = {}
        num_od = dict()
        for demo in self.expert_trajectories_per_pr[profile]:
            ...
            if od not in Ds:
                # It should never be bigger than horizon expert routes
                Ds[od] = np.zeros((ldemo+1, n_states))
            ...
            for t in range(ldemo):
                for s in self.env.valid_edges:
                    ...
                    # @ T[sprev, :, s] #pi already accounts for T.
                    Ds[od][t + 1,
                           s] += np.dot(Ds[od][t, sprev], pi[des, sprev, acts_to_s])
            num_od[od] += 1

            # assert isinstance(Dcum, np.ndarray)
        for (orig, des), Dsval in Ds.items():
            Dcums[((orig, des), tuple(profile))] = Dsval.sum(
                axis=0)/num_od[(orig, des)]

        return Ds, Dcums

    def _set_demo_oms_from_trajectories(self, trajs: Iterable[types.Trajectory]) -> None:

        self.demo_state_om_per_profile = get_demo_oms_from_trajectories(
            trajs, state_dim=self.env.state_dim, discount=self.discount)
        ...
        if self.use_dijkstra:
            if use_cached_policies:
                if profile not in self.policies_per_profile.keys():
                    reward_matrix = self.calculate_rewards_for_destinations_and_profile(
                        self.destinations if on_destinations is None else on_destinations, profile)
            ...
            self.default_vi_policy.value_iteration(error=0.1,
                                                   profile=self.env.last_profile if profile is None else tuple(
                                                       profile),
    ...
    def mce_partition_fh(self, reward_matrix, profile=None, od_list=None):
        ...
        if profile in self.training_profiles:
            return self.vi_policy_per_profile[profile].value_iteration(error=1,
                                                                       profile=self.env.last_profile if profile is None else tuple(
                                                                           profile),
                                                                       custom_reward=self.reward_from_matrix(
                                                                           reward_matrix),
    ...
    def expected_trajectory_cost_calculation(self, on_profiles, learned_profiles, costs_measured_with_profiles=BASIC_PROFILES, stochastic_sampling=False, n_samples_per_od=None, custom_cost_preprocessing=None, repeat_society=10, bins=30, new_test_data=0, name_method='', plot_histograms=False, plot_learned_profiles_as_probabilities=False):
        ...
        n_samples_per_od = self.n_repeat_per_od_monte_carlo if n_samples_per_od is None else int(
            n_samples_per_od)
        ...
        train_rows.update({f'expert_pr_{tuple(float(f"{t:0.2f}") for t in pr)}': {
                          pr_2: (0, 0) for pr_2 in measure_profiles} for pr in on_profiles})
        train_rows.update({f'society_pr_{tuple(float(f"{t:0.2f}") for t in pr)}': {
                          pr_2: (0, 0) for pr_2 in measure_profiles} for pr in on_profiles})
        train_rows.update({f'learned_pr_{tuple(float(f"{t:0.2f}") for t in target_pr)}_result_{tuple(float(f"{t:0.2f}") for t in learned_pr)}': {
                          pr_2: (0, 0) for pr_2 in measure_profiles} for learned_pr, target_pr in learned_profiles})
        if plot_learned_profiles_as_probabilities:
            train_rows.update({f'learned_pr_probability_{tuple(float(f"{t:0.2f}") for t in target_pr)}_result_{tuple(float(f"{t:0.2f}") for t in learned_pr)}': {
                              pr_2: (0, 0) for pr_2 in measure_profiles} for learned_pr, target_pr in learned_profiles})
        ...
        for learned_pr, target_pr in learned_profiles:
            ...
            self.adapt_policy_to_profile(
                learned_pr, use_cached_policies=True, on_destinations=destinations_all)
            # assert self._reward_net.cur_profile == learned_pr
            # Profile learned.
            sampler_learned_profile: SimplePolicy = SimplePolicy.from_sb3_policy(
                policy=self.policy, real_env=self.env)
            learned_profile_trajs_train = sampler_learned_profile.sample_trajectories(
                stochastic=stochastic_sampling, repeat_per_od=n_samples_per_od, with_profiles=[learned_pr,], od_list=self.od_list_train)
            learned_profile_trajs_test = sampler_learned_profile.sample_trajectories(
                stochastic=stochastic_sampling, repeat_per_od=n_samples_per_od, with_profiles=[learned_pr,], od_list=od_list_test_new)
            learned_profile_as_probs_trajs_train = []
            learned_profile_as_probs_trajs_test = []
            society_trajs_train = []
            society_trajs_test = []

            expert_trajs_train = expert_policy.sample_trajectories(
                stochastic=self.stochastic_expert, repeat_per_od=n_samples_per_od, with_profiles=[target_pr,], od_list=self.od_list_train)
            assert len(expert_trajs_train) > 0

            expert_trajs_test = expert_policy.sample_trajectories(
                stochastic=self.stochastic_expert, repeat_per_od=n_samples_per_od, with_profiles=[target_pr,], od_list=od_list_test_new)
            assert len(expert_trajs_test) > 0

            for _ in range(repeat_society):
                od_per_pr_train = self.divide_od_list_as_per_profile(
                    self.od_list_train, target_pr)
                od_per_pr_test = self.divide_od_list_as_per_profile(
                    od_list_test_new, target_pr)
                for bpr in od_per_pr_test.keys():
                    ...
                    society_trajs_train.extend(expert_policy.sample_trajectories(
                        stochastic=self.stochastic_expert, repeat_per_od=n_samples_per_od, with_profiles=[bpr,], od_list=od_per_pr_train[bpr]))
                    society_trajs_test.extend(expert_policy.sample_trajectories(
                        stochastic=self.stochastic_expert, repeat_per_od=n_samples_per_od, with_profiles=[bpr,], od_list=od_per_pr_test[bpr]))

                if plot_learned_profiles_as_probabilities:
                    for bpr in od_per_pr_test.keys():
                        self.adapt_policy_to_profile(
                            bpr, use_cached_policies=True, on_destinations=destinations_all)
                        sampler: SimplePolicy = SimplePolicy.from_sb3_policy(
                            policy=self.policy, real_env=self.env)

                        learned_profile_as_probs_trajs_train.extend(sampler.sample_trajectories(
                            stochastic=stochastic_sampling, repeat_per_od=n_samples_per_od, with_profiles=[bpr,], od_list=od_per_pr_train[bpr]))
                        learned_profile_as_probs_trajs_test.extend(sampler.sample_trajectories(
                            stochastic=stochastic_sampling, repeat_per_od=n_samples_per_od, with_profiles=[bpr,], od_list=od_per_pr_test[bpr]))
            ...
            costs_learned_and_expert_test, agou_learned_and_expert_test_target = calculate_expected_similarities_and_std(
                self.env.cost_model, profile_for_cost_model=target_pr, sample_trajs=learned_profile_trajs_test, expert_trajs=expert_trajs_test, profile_criteria=costs_measured_with_profiles, preprocessing=custom_cost_preprocessing)
            ...
            jaccard_per_pr_expert_train, jaccard_expert_train_normal, jaccard_expert_train_costs, _ = jaccard_similarity(
                self.env.cost_model, profile_for_cost_model=target_pr, sample_trajs=learned_profile_trajs_train, expert_trajs=expert_trajs_train, profile_criteria=costs_measured_with_profiles, preprocessing=custom_cost_preprocessing, return_per_od=True)
            jaccard_per_pr_expert_test, jaccard_expert_test_normal, jaccard_expert_test_costs, _ = jaccard_similarity(
                self.env.cost_model, profile_for_cost_model=target_pr, sample_trajs=learned_profile_trajs_test, expert_trajs=expert_trajs_test, profile_criteria=costs_measured_with_profiles, preprocessing=custom_cost_preprocessing, return_per_od=True)

            jaccard_per_pr_society_train, jaccard_society_train_normal, jaccard_society_train_costs, _ = jaccard_similarity(
                self.env.cost_model, profile_for_cost_model=target_pr, sample_trajs=learned_profile_trajs_train, expert_trajs=society_trajs_train, profile_criteria=costs_measured_with_profiles, preprocessing=custom_cost_preprocessing, return_per_od=True)
            jaccard_per_pr_society_test, jaccard_society_test_normal, jaccard_society_test_costs, _ = jaccard_similarity(
                self.env.cost_model, profile_for_cost_model=target_pr, sample_trajs=learned_profile_trajs_test, expert_trajs=society_trajs_test, profile_criteria=costs_measured_with_profiles, preprocessing=custom_cost_preprocessing, return_per_od=True)

            demo_oms_learned_profile_trajs = get_demo_oms_from_trajectories(
                learned_profile_trajs_train, state_dim=self.env.state_dim, groupby='od')
            demo_oms_expert_trajs = get_demo_oms_from_trajectories(
                expert_trajs_train, state_dim=self.env.state_dim, groupby='od')
            demo_oms_society_trajs = get_demo_oms_from_trajectories(
                society_trajs_train, state_dim=self.env.state_dim, groupby='od')
            keys_vc = demo_oms_learned_profile_trajs.keys()

            visitation_diff_expert_train = np.sum([np.abs(
                demo_oms_learned_profile_trajs[od] - demo_oms_expert_trajs[od]) for od in keys_vc], axis=1)
            visitation_diff_society_train = np.sum([np.abs(
                demo_oms_learned_profile_trajs[od] - demo_oms_society_trajs[od]) for od in keys_vc], axis=1)

            demo_oms_learned_profile_trajs = get_demo_oms_from_trajectories(
                learned_profile_trajs_test, state_dim=self.env.state_dim, groupby='od')
            demo_oms_expert_trajs = get_demo_oms_from_trajectories(
                expert_trajs_test, state_dim=self.env.state_dim, groupby='od')
            demo_oms_society_trajs = get_demo_oms_from_trajectories(
                society_trajs_test, state_dim=self.env.state_dim, groupby='od')
            keys_vc = demo_oms_learned_profile_trajs.keys()
            visitation_diff_expert_test = np.sum([np.abs(
                demo_oms_learned_profile_trajs[od] - demo_oms_expert_trajs[od]) for od in keys_vc], axis=1)
            visitation_diff_society_test = np.sum([np.abs(
                demo_oms_learned_profile_trajs[od] - demo_oms_society_trajs[od]) for od in keys_vc], axis=1)

            similarity_agou_now_train = {
                'Learned Profile': learned_pr, 'Target Profile': target_pr, }
            similarity_agou_now_test = {
                'Learned Profile': learned_pr, 'Target Profile': target_pr, }
            similarity_agou_now_train['Similarity with target profile agent'] = np.mean(
                agou_learned_and_expert_train_target), np.std(agou_learned_and_expert_train_target)
            similarity_agou_now_test['Similarity with target profile agent'] = np.mean(
                agou_learned_and_expert_test_target), np.std(agou_learned_and_expert_test_target)
            similarity_agou_now_train['Similarity with target profile society'] = np.mean(
                agou_learned_and_society_train_target), np.std(agou_learned_and_society_train_target)
            similarity_agou_now_test['Similarity with target profile society'] = np.mean(
                agou_learned_and_society_test_target), np.std(agou_learned_and_society_test_target)

            similarity_vc_now_train = {
                'Learned Profile': learned_pr, 'Target Profile': target_pr, }
            similarity_vc_now_test = {
                'Learned Profile': learned_pr, 'Target Profile': target_pr, }
            similarity_vc_now_train['Similarity with target profile agent'] = np.mean(
                visitation_diff_expert_train), np.std(visitation_diff_expert_train)
            similarity_vc_now_test['Similarity with target profile agent'] = np.mean(
                visitation_diff_expert_test), np.std(visitation_diff_expert_test)
            similarity_vc_now_train['Similarity with target profile society'] = np.mean(
                visitation_diff_society_train), np.std(visitation_diff_society_train)
            similarity_vc_now_test['Similarity with target profile society'] = np.mean(
                visitation_diff_society_test), np.std(visitation_diff_society_test)

            similarity_jaccard_now_train = {
                'Learned Profile': learned_pr, 'Target Profile': target_pr, }
            similarity_jaccard_now_test = {
                'Learned Profile': learned_pr, 'Target Profile': target_pr, }
            similarity_jaccard_now_train['Similarity with target profile agent'] = np.mean(
                jaccard_expert_train_costs), np.std(jaccard_expert_train_costs)
            similarity_jaccard_now_test['Similarity with target profile agent'] = np.mean(
                jaccard_expert_test_costs), np.std(jaccard_expert_test_costs)
            similarity_jaccard_now_train['Similarity with target profile society'] = np.mean(
                jaccard_society_train_costs), np.std(jaccard_society_train_costs)
            similarity_jaccard_now_test['Similarity with target profile society'] = np.mean(
                jaccard_society_test_costs), np.std(jaccard_society_test_costs)

            for ipr_2, pr_2 in enumerate(measure_profiles):
                assert np.allclose(costs_learned_and_expert_train['sample'][:, ipr_2], np.dot(
                    costs_learned_and_expert_train['sample'], np.asarray(pr_2)))
                ...
                agou_pr_2_train = profiled_aggregated_similarity_Agourogiannis_et_al_2023(
                    costs_learned_and_expert_train['sample'], costs_learned_and_expert_train['expert'], preference_weights=pr_2)
                agou_pr_2_test = profiled_aggregated_similarity_Agourogiannis_et_al_2023(
                    costs_learned_and_expert_test['sample'], costs_learned_and_expert_test['expert'], preference_weights=pr_2)

                jaccard_pr_2_train = jaccard_per_pr_expert_train[pr_2]
                jaccard_pr_2_test = jaccard_per_pr_expert_test[pr_2]

                similarity_agou_now_train[f'Similarity in terms of {str.capitalize(BASIC_PROFILE_NAMES.get(pr_2, tuple(float(f"{t:0.2f}") for t in pr_2)))}'] = np.mean(
                    agou_pr_2_train), np.std(agou_pr_2_train)
                similarity_agou_now_test[f'Similarity in terms of {str.capitalize(BASIC_PROFILE_NAMES.get(pr_2, tuple(float(f"{t:0.2f}") for t in pr_2)))}'] = np.mean(
                    agou_pr_2_test), np.std(agou_pr_2_test)
                ...
                similarity_jaccard_now_train[f'Similarity in terms of {str.capitalize(BASIC_PROFILE_NAMES.get(pr_2, tuple(float(f"{t:0.2f}") for t in pr_2)))}'] = np.mean(
                    jaccard_pr_2_train), np.std(jaccard_pr_2_train)
                similarity_jaccard_now_test[f'Similarity in terms of {str.capitalize(BASIC_PROFILE_NAMES.get(pr_2, tuple(float(f"{t:0.2f}") for t in pr_2)))}'] = np.mean(
                    jaccard_pr_2_test), np.std(jaccard_pr_2_test)

                # self.adapt_policy_to_profile(pr_2, use_cached_policies=True, on_destinations=destinations_all)
                if pr_2 in BASIC_PROFILES:
                    expert_mpr_train = expert_policy.sample_trajectories(
                        stochastic=self.stochastic_expert, repeat_per_od=n_samples_per_od, with_profiles=[pr_2,], od_list=self.od_list_train)
                    expert_mpr_test = expert_policy.sample_trajectories(
                        stochastic=self.stochastic_expert, repeat_per_od=n_samples_per_od, with_profiles=[pr_2,], od_list=od_list_test_new)

                    costs_mpr_train, agou_mpr_train = calculate_expected_similarities_and_std(
                        self.env.cost_model, profile_for_cost_model=target_pr, sample_trajs=learned_profile_trajs_train, expert_trajs=expert_mpr_train, profile_criteria=costs_measured_with_profiles, preprocessing=custom_cost_preprocessing)
                    ...
                    agou_mpr_train = profiled_aggregated_similarity_Agourogiannis_et_al_2023(
                        costs_mpr_train['sample'], costs_mpr_train['expert'], preference_weights=pr_2)
                    agou_mpr_test = profiled_aggregated_similarity_Agourogiannis_et_al_2023(
                        costs_mpr_test['sample'], costs_mpr_test['expert'], preference_weights=pr_2)
                    similarity_agou_now_train[f'Similarity with {str.capitalize(BASIC_PROFILE_NAMES.get(pr_2, tuple(float(f"{t:0.2f}") for t in pr_2)))} expert'] = (
                        np.mean(agou_mpr_train), np.std(agou_mpr_train))
                    similarity_agou_now_test[f'Similarity with {str.capitalize(BASIC_PROFILE_NAMES.get(pr_2, tuple(float(f"{t:0.2f}") for t in pr_2)))} expert'] = (
                        np.mean(agou_mpr_test), np.std(agou_mpr_test))

                    demo_oms_learned_profile_trajs_test = get_demo_oms_from_trajectories(
                        learned_profile_trajs_test, state_dim=self.env.state_dim, groupby='od')
                    demo_oms_learned_profile_trajs_train = get_demo_oms_from_trajectories(
                        learned_profile_trajs_train, state_dim=self.env.state_dim, groupby='od')

                    demo_oms_expert_trajs_train = get_demo_oms_from_trajectories(
                        expert_mpr_train, state_dim=self.env.state_dim, groupby='od')
                    demo_oms_expert_trajs_test = get_demo_oms_from_trajectories(
                        expert_mpr_test, state_dim=self.env.state_dim, groupby='od')
                    keys_vc_train = demo_oms_learned_profile_trajs_train.keys()
                    keys_vc_test = demo_oms_learned_profile_trajs_test.keys()
                    visitation_diff_expert_test = np.sum([np.abs(
                        demo_oms_learned_profile_trajs_test[od] - demo_oms_expert_trajs_test[od]) for od in keys_vc_test], axis=1)
                    visitation_diff_expert_train = np.sum([np.abs(
                        demo_oms_learned_profile_trajs_train[od] - demo_oms_expert_trajs_train[od]) for od in keys_vc_train], axis=1)

                    similarity_vc_now_train[f'Similarity with {str.capitalize(BASIC_PROFILE_NAMES.get(pr_2, tuple(float(f"{t:0.2f}") for t in pr_2)))} expert'] = (
                        np.mean(visitation_diff_expert_train), np.std(visitation_diff_expert_train))
                    similarity_vc_now_test[f'Similarity with {str.capitalize(BASIC_PROFILE_NAMES.get(pr_2, tuple(float(f"{t:0.2f}") for t in pr_2)))} expert'] = (
                        np.mean(visitation_diff_expert_test), np.std(visitation_diff_expert_test))

                    jaccard_per_pr_expert, _, jaccard_expert_cost_train, _ = jaccard_similarity(
                        self.env.cost_model, profile_for_cost_model=pr_2, sample_trajs=learned_profile_trajs_train, expert_trajs=expert_mpr_train, profile_criteria=costs_measured_with_profiles, preprocessing=custom_cost_preprocessing, return_per_od=True)
                    jaccard_per_pr_expert, _, jaccard_expert_cost_test, _ = jaccard_similarity(
                        self.env.cost_model, profile_for_cost_model=pr_2, sample_trajs=learned_profile_trajs_test, expert_trajs=expert_mpr_test, profile_criteria=costs_measured_with_profiles, preprocessing=custom_cost_preprocessing, return_per_od=True)

                    similarity_jaccard_now_train[f'Similarity with {str.capitalize(BASIC_PROFILE_NAMES.get(pr_2, tuple(float(f"{t:0.2f}") for t in pr_2)))} expert'] = (
                        np.mean(jaccard_expert_cost_train), np.std(jaccard_expert_cost_train))
                    similarity_jaccard_now_test[f'Similarity with {str.capitalize(BASIC_PROFILE_NAMES.get(pr_2, tuple(float(f"{t:0.2f}") for t in pr_2)))} expert'] = (
                        np.mean(jaccard_expert_cost_test), np.std(jaccard_expert_cost_test))

                avg_cost_learned_profile_trajs_train, std_learned_profile_trajs_train = np.mean(
                    costs_learned_and_expert_train['sample'][:, ipr_2]), np.std(costs_learned_and_expert_train['sample'][:, ipr_2])
                avg_cost_learned_profile_trajs_test, std_learned_profile_trajs_test = np.mean(
                    costs_learned_and_expert_test['sample'][:, ipr_2]), np.std(costs_learned_and_expert_test['sample'][:, ipr_2])
                ...
                box_plots_train[f'learned_pr_{tuple(float(f"{t:0.2f}") for t in target_pr)}_result_{tuple(float(f"{t:0.2f}") for t in learned_pr)}'][
                    pr_2] = costs_learned_and_expert_train['sample'][:, ipr_2]
                box_plots_test[f'learned_pr_{tuple(float(f"{t:0.2f}") for t in target_pr)}_result_{tuple(float(f"{t:0.2f}") for t in learned_pr)}'][
                    pr_2] = costs_learned_and_expert_test['sample'][:, ipr_2]

                train_rows[f'learned_pr_{tuple(float(f"{t:0.2f}") for t in target_pr)}_result_{tuple(float(f"{t:0.2f}") for t in learned_pr)}'][pr_2] = (
                    avg_cost_learned_profile_trajs_train, std_learned_profile_trajs_train)
                test_rows[f'learned_pr_{tuple(float(f"{t:0.2f}") for t in target_pr)}_result_{tuple(float(f"{t:0.2f}") for t in learned_pr)}'][pr_2] = (
                    avg_cost_learned_profile_trajs_test, std_learned_profile_trajs_test)

                avg_train, std_train = np.mean(costs_learned_and_expert_train['expert'][:, ipr_2]), np.std(
                    costs_learned_and_expert_train['expert'][:, ipr_2])
                avg_test, std_test = np.mean(costs_learned_and_expert_test['expert'][:, ipr_2]), np.std(
                    costs_learned_and_expert_test['expert'][:, ipr_2])

                box_plots_train[f'expert_pr_{tuple(float(f"{t:0.2f}") for t in target_pr)}'][
                    pr_2] = costs_learned_and_expert_train['expert'][:, ipr_2]
                box_plots_test[f'expert_pr_{tuple(float(f"{t:0.2f}") for t in target_pr)}'][
                    pr_2] = costs_learned_and_expert_test['expert'][:, ipr_2]

                train_rows[f'expert_pr_{tuple(float(f"{t:0.2f}") for t in target_pr)}'][pr_2] = (
                    avg_train, std_train)
                test_rows[f'expert_pr_{tuple(float(f"{t:0.2f}") for t in target_pr)}'][pr_2] = (
                    avg_test, std_test)

                avg_train, std_train = np.mean(costs_learned_and_society_train['expert'][:, ipr_2]), np.std(
                    costs_learned_and_society_train['expert'][:, ipr_2])
                avg_test, std_test = np.mean(costs_learned_and_society_test['expert'][:, ipr_2]), np.std(
                    costs_learned_and_society_test['expert'][:, ipr_2])

                box_plots_train[f'society_pr_{tuple(float(f"{t:0.2f}") for t in target_pr)}'][
                    pr_2] = costs_learned_and_society_train['expert'][:, ipr_2]
                box_plots_test[f'society_pr_{tuple(float(f"{t:0.2f}") for t in target_pr)}'][
                    pr_2] = costs_learned_and_society_test['expert'][:, ipr_2]

                train_rows[f'society_pr_{tuple(float(f"{t:0.2f}") for t in target_pr)}'][pr_2] = (
                    avg_train, std_train)
                test_rows[f'society_pr_{tuple(float(f"{t:0.2f}") for t in target_pr)}'][pr_2] = (
                    avg_test, std_test)
                ...
                if plot_learned_profiles_as_probabilities:
                    ...
                    train_rows[f'learned_pr_probability_{tuple(float(f"{t:0.2f}") for t in target_pr)}_result_{tuple(float(f"{t:0.2f}") for t in learned_pr)}'][pr_2] = (
                        avg_cost_learned_probs_profile_trajs_train, std_learned_probs_profile_trajs_train)
                    test_rows[f'learned_pr_probability_{tuple(float(f"{t:0.2f}") for t in target_pr)}_result_{tuple(float(f"{t:0.2f}") for t in learned_pr)}'][pr_2] = (
                        avg_cost_learned_probs_profile_trajs_test, std_learned_probs_profile_trajs_test)
            ...
            similarity_data_train['visitation_count'].append(
                similarity_vc_now_train)
            similarity_data_train['jaccard'].append(
                similarity_jaccard_now_train)

            similarity_data_test['agou'].append(similarity_agou_now_test)
            similarity_data_test['visitation_count'].append(
                similarity_vc_now_test)
        ...
        for pr in measure_profiles:
            self.adapt_policy_to_profile(
                pr, use_cached_policies=True, on_destinations=destinations_all)
            sampler: SimplePolicy = SimplePolicy.from_sb3_policy(
                policy=self.policy, real_env=self.env)

            value_learned_trajs_train = sampler.sample_trajectories(
                stochastic=stochastic_sampling, repeat_per_od=n_samples_per_od, with_profiles=[pr,], od_list=self.od_list_train)

            value_learned_trajs_test = sampler.sample_trajectories(
                stochastic=stochastic_sampling, repeat_per_od=n_samples_per_od, with_profiles=[pr,], od_list=od_list_test_new)

            expert_trajs_train_pr = expert_policy.sample_trajectories(
                stochastic=self.stochastic_expert, repeat_per_od=n_samples_per_od, with_profiles=[pr,], od_list=self.od_list_train)
            expert_trajs_test_pr = expert_policy.sample_trajectories(
                stochastic=self.stochastic_expert, repeat_per_od=n_samples_per_od, with_profiles=[pr,], od_list=od_list_test_new)
            ...
            train_rows['expert'][pr] = (
                avg_cost_expert_trajs_train, std_expert_trajs_train)
            box_plots_train['expert'][pr] = allcosts_ettr

            train_rows['learned'][pr] = (
                avg_cost_learned_trajs_train, std_learned_trajs_train)
            box_plots_train['learned'][pr] = allcosts_lttr
            test_rows['expert'][pr] = (
                avg_cost_expert_trajs_test, std_expert_trajs_test)
            box_plots_test['expert'][pr] = allcosts_ette
            test_rows['learned'][pr] = (
                avg_cost_learned_trajs_test, std_learned_trajs_test)
        ...
        for i, pr in enumerate(measure_profiles):
            ...
            positions = np.array(list((range(len(boxes))))) + \
                i*1.0/len(measure_profiles)
            bp = plt.boxplot(boxes, positions=positions,
                             patch_artist=True, widths=0.7*1.0/len(measure_profiles))
            ...
            if i == len(measure_profiles)-1:
                for p in positions:
                    ...
                    plt.axvline(x=(p + p + 1.0/len(measure_profiles)
                                   )/2, color='blue', linestyle='--')

        title = f"Test set: Boxplot (cost distribution calculated with different measure_profiles)"
        plt.title(title)
        plt.xticks(np.asarray(range(len(box_plots_train))) + (len(measure_profiles)-1) /
                   2.0/len(measure_profiles), labels=xlabels, rotation=75)
        plt.tight_layout()
        plt.savefig(
            f'results/value_system_identification/{name_method}_boxplot_train.pdf', bbox_inches="tight")
        # plt.show()
        plt.close()
        ...
        for i, pr in enumerate(measure_profiles):
            ...
            bp = plt.boxplot(boxes, positions=np.array(list((range(len(boxes))))) + i*1.0/len(
                measure_profiles), patch_artist=True, widths=0.7*1.0/len(measure_profiles))
            for patch in bp['boxes']:
                # color2 = 'r' if color == 'red' else 'g' if color == 'green' else 'b' if color == 'blue' else 'w'
                patch.set_facecolor(PROFILE_COLORS.get(pr, 'w'))
                # patch.set_edgecolor(PROFILE_COLORS.get(pr, 'w'))
                if i == len(measure_profiles)-1:
                    for p in positions:
                        # Highlighting the minimum value
                        plt.axvline(
                            x=(p + p + 1.0/len(measure_profiles))/2, color='blue', linestyle='--')

        title = f"Test set: Boxplot (cost distribution calculated with different measure_profiles)"
        plt.title(title)
        plt.xticks(np.asarray(range(len(box_plots_test))) + (len(measure_profiles)-1) /
                   2.0/len(measure_profiles), labels=xlabels, rotation=75)
        plt.tight_layout()
        plt.savefig(
            f'results/value_system_identification/{name_method}_boxplot_test.pdf', bbox_inches="tight")
        # plt.show()
        plt.close()
        ...
        for iprk, prkey in enumerate(measure_profiles):
            ...
            labels = []
            for key in box_plots_train.keys():
                boxplot_train_of_prkey.append(
                    list(box_plots_train[key][prkey]))
            ...
            plt.title(title)
            plt.xticks(list(i + 0.5 for i in range(len(xlabels))),
                       labels=xlabels, rotation=75)
            plt.tight_layout()
            plt.savefig(
                f'results/value_system_identification/{name_method}_boxplot_test_costs_calculated_with_{prkey}.pdf', bbox_inches="tight")
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig(
                f'results/value_system_identification/{name_method}_boxplot_train_costs_calculated_with_{prkey}.pdf', bbox_inches="tight")
            plt.close()

        for key in train_rows.keys():
            keyname = get_key_name(key)
            profile_data = {'Policy': keyname}
            profile_data.update({BASIC_PROFILE_NAMES.get(pr, tuple(float(
                f"{t:0.2f}") for t in pr)): train_rows[key][pr] for pr in measure_profiles})
        ...
        for key in test_rows.keys():
            ...
            profile_data = {'Policy': keyname}
            profile_data.update({BASIC_PROFILE_NAMES.get(pr, tuple(
                float(f"{t:0.2f}") for t in pr)): test_rows[key][pr] for pr in measure_profiles})
        ...
        for metric in similarity_metrics:
            similarity_data_test[metric] = pd.DataFrame(
                data=similarity_data_test[metric], columns=similarity_columns)
            similarity_data_train[metric] = pd.DataFrame(
                data=similarity_data_train[metric], columns=similarity_columns)
    ...
    def feature_differences(self, sampled_trajs, obs_matrix, per_state=False, visitations=None, use_info_real_costs=True, sampled_traj_profile_to_expert_traj_profile_mapping={}):
        ...
        for traj in sampled_trajs:
            ...
            odpr_of_sampled_trajs = (
                (traj.infos[0]['orig'], traj.infos[0]['des']), pr)
            expert_pr = profile_mapping.get(pr, pr)
            corresponding_odpr_of_expert_trajs = (
                (traj.infos[0]['orig'], traj.infos[0]['des']), expert_pr)

            expert_trajs_of_corresponding_odpr = self.expert_trajectories_per_odpr[
                corresponding_odpr_of_expert_trajs]
            ...
            if per_state is False:
                ...
                if use_info_real_costs is False:
                    traj_obs = np.asarray(
                        [obs_matrix_np[o[0], traj.infos[0]['des'], :] for o in traj.obs])
                else:
                    traj_obs = np.asarray([self.env.cost_model(
                        expert_pr, normalization=FeaturePreprocess.NORMALIZATION)(traj.obs[i]) for i in range(len(traj))])

                for expert_traj in expert_trajs_of_corresponding_odpr:
                    if use_info_real_costs is False:
                        expert_traj_obs = np.asarray(
                            [obs_matrix_np[o[0], expert_traj.infos[0]['des'], :] for o in expert_traj.obs])
                    else:
                        expert_traj_obs = np.asarray([self.env.cost_model(expert_pr, normalization=FeaturePreprocess.NORMALIZATION)(
                            expert_traj.obs[i]) for i in range(len(expert_traj))])

                    fd_diff = np.sum(traj_obs, axis=0) - \
                        np.sum(expert_traj_obs, axis=0)
                ...
                traj_obs = np.multiply(
                    obs_matrix_np[:, traj.infos[0]['des'], :], visitations[odpr_of_sampled_trajs].reshape(-1, 1))
                expected_expert_traj_obs = np.zeros_like(
                    obs_matrix_np[:, expert_trajs_of_corresponding_odpr[0].infos[0]['des'], :])
                for expert_traj in expert_trajs_of_corresponding_odpr:
                    expert_traj_obs = np.multiply(
                        obs_matrix_np[:, expert_traj.infos[0]['des'], :], self.demo_state_om_per_profile[corresponding_odpr_of_expert_trajs].reshape(-1, 1))
                    expected_expert_traj_obs += expert_traj_obs
                expected_expert_traj_obs /= len(
                    expert_trajs_of_corresponding_odpr)

                fd_norm = np.linalg.norm(
                    traj_obs - expected_expert_traj_obs, axis=-1)
    ...
    def calculate_rewards_for_destinations_and_profile(self, destinations, chosen_profile, obs_mat=None, dones=None):
        ...
        if dones is None:
            dones = th.as_tensor(self.env.done_matrix,
                                 dtype=self._reward_net.dtype)

        state_space_is_1d = isinstance(
            self.env.state_space, gym.spaces.Discrete)
        ...
        with th.no_grad():
            ...
            if state_space_is_1d:
                predicted_r = squeeze_r(self._reward_net(
                    None, None, obs_mat, dones[:, destinations[0]]))

                assert predicted_r.shape == (obs_mat.shape[0],)

                predicted_r_np = predicted_r.detach().cpu().numpy()
            else:
                predicted_r_np = np.zeros_like(
                    self.env.reward_matrix[list(self.env.reward_matrix.keys())[0]])
                for d in destinations:
                    predicted_r = squeeze_r(self._reward_net(
                        None, None, obs_mat[:, d, :], dones[:, d]))
    ...
    def _train_step_ponderated_profiles(self, obs_mat: th.Tensor, dones: th.Tensor = None, od_train_per_profile: dict = None, profile_ponderation: list = None, loss_weighting=1.0) -> Tuple[np.ndarray, np.ndarray]:
        ...
        predicted_r_np = self.calculate_rewards_for_destinations_and_profile(
            self.destinations, learned_policy_profile, obs_mat=obs_mat, dones=dones)

        if self.use_mce_om:
            pi = self.mce_partition_fh(
                predicted_r_np, profile=learned_policy_profile)
            ...
            sampler: SimplePolicy = SimplePolicy.from_policy_matrix(
                pi, real_env=self.env)
            sampled_trajs = sampler.sample_trajectories(
                stochastic=self.stochastic_expert, repeat_per_od=self.n_repeat_per_od_monte_carlo, with_profiles=[learned_policy_profile,])
            ...
            visitations = get_demo_oms_from_trajectories(
                sampled_trajs, state_dim=self.env.state_dim)
        ...
        for chosen_profile in BASIC_PROFILES:
            ...
            chosen_profile_expert_trajs_train = [t for t in self.expert_trajectories if tuple(t.infos[0]['profile']) == tuple(
                chosen_profile) and (t.infos[0]['orig'], t.infos[0]['des']) in od_train_per_profile[chosen_profile]]
            od_profiles_of_trajs_train = [
                ((t.infos[0]['orig'], t.infos[0]['des']), chosen_profile) for t in chosen_profile_expert_trajs_train]
            ...
            fd_normed_tensor_train = th.as_tensor(np.asarray([fd_normed_train[(
                od, chosen_profile)] for od in od_train_per_profile[chosen_profile]]), device=self._reward_net.device, dtype=self._reward_net.dtype)
            ...
            mntensor_train_per_pr[chosen_profile] = (th.mean(
                fd_normed_tensor_train)*profile_ponderation[chosen_profile]).detach().numpy()
            ...
            fd_normed_tensor_test = th.as_tensor(np.asarray([fd_normed_test[(
                od, chosen_profile)] for od in self.od_list_test]), device=self._reward_net.device, dtype=self._reward_net.dtype)
            ...
            mntensor_test_per_pr[chosen_profile] = (th.mean(
                fd_normed_tensor_test)*profile_ponderation[chosen_profile]).detach().numpy()

            obs_matrix_all_trajs = th.vstack(
                [obs_mat[:, t.infos[0]['des'], :] for t in chosen_profile_expert_trajs_train])
            done_matrix_all_trajs = th.vstack(
                [dones[:, t.infos[0]['des']] for t in chosen_profile_expert_trajs_train])

            rewards = self._reward_net(
                None, None, obs_matrix_all_trajs, done_matrix_all_trajs).reshape_as(visitation_count_diffs)
            ...
            if self.fd_lambda > 0.0:
                visited_states_for_each_od = (
                    th.as_tensor(
                        np.asarray([visitations[(odpr[0], learned_policy_profile)]
                                    for odpr in od_profiles_of_trajs_train]),
                ...
                losses = th.mul(
                    visitation_count_diffs + self.fd_lambda /
                    (2*len(chosen_profile_expert_trajs_train))*fd_on_visited_states,
            ...
            if wheres[0].shape[0] <= 1:
                ...
                # /len([odpr for odpr in od_profiles_of_trajs_train if losses[:,odpr[0][1]]]) #/len([t for t in self.expert_trajectories if tuple(t.infos[0]['profile']) == tuple(chosen_profile)])
                loss = th.sum(losses[wheres])/len(set(wheres[0].tolist()))
    ...
    def _train_step_sampling_trajectories(self, obs_mat: th.Tensor, dones: th.Tensor = None, society_profiles: list = [], society_profile_distribution: np.ndarray = np.array([]), batch_size=100) -> Tuple[np.ndarray, np.ndarray]:
        ...
        with th.no_grad():
            ...
            for pr in BASIC_PROFILES:
                pure_rewards_per_pure_pr[pr] = self.calculate_rewards_for_destinations_and_profile(
                    self.destinations, pr, obs_mat=obs_mat, dones=dones)
        ...
        # weights_th(s) = \pi(s) - D(s)
            ...
            ods_index = np.random.choice(
                len(self.od_list_train), size=batch_size, replace=True)
            ods = [(od[0], od[1]) for od in od_list_array[ods_index].tolist()]
            profiles_index = np.random.choice(len(
                society_profiles), size=batch_size, p=society_profile_distribution, replace=True)

            profiles = [tuple(pr)
                        for pr in society_array[profiles_index].tolist()]

            ods_index_test = np.random.choice(
                len(self.od_list_test), size=batch_size, replace=True)
            ods_test = [(od[0], od[1])
                        for od in od_list_array_test[ods_index_test].tolist()]
            profiles_index_test = np.random.choice(len(
                society_profiles), size=batch_size, p=society_profile_distribution, replace=True)

            profiles_test = [
                tuple(pr) for pr in society_array[profiles_index_test].tolist()]
            ...
            for od, pr in zip(ods, profiles):
                learned_policy_trajs.extend(sampler.sample_trajectories(
                    stochastic=self.stochastic_expert, repeat_per_od=self.n_repeat_per_od_monte_carlo, with_profiles=[learned_policy_profile,], od_list=[od,]))

                expert_policy_trajs.extend(self.expert_policy.sample_trajectories(
                    stochastic=self.stochastic_expert, repeat_per_od=self.n_repeat_per_od_monte_carlo, with_profiles=[pr,], od_list=[od,]))

            for od, pr in zip(ods_test, profiles_test):
                learned_policy_trajs_test.extend(sampler.sample_trajectories(
                    stochastic=self.stochastic_expert, repeat_per_od=self.n_repeat_per_od_monte_carlo, with_profiles=[learned_policy_profile,], od_list=[od,]))

                expert_policy_trajs_test.extend(self.expert_policy.sample_trajectories(
                    stochastic=self.stochastic_expert, repeat_per_od=self.n_repeat_per_od_monte_carlo, with_profiles=[pr,], od_list=[od,]))

            sampled_trajs_train = sampler.sample_trajectories(stochastic=self.stochastic_expert, repeat_per_od=self.n_repeat_per_od_monte_carlo, with_profiles=[
                                                              learned_policy_profile,], od_list=self.od_list_train)
            sampled_trajs_test = sampler.sample_trajectories(stochastic=self.stochastic_expert, repeat_per_od=self.n_repeat_per_od_monte_carlo, with_profiles=[
                                                             learned_policy_profile,], od_list=self.od_list_test)

            all_trajs = deepcopy(sampled_trajs_train)
            all_trajs.extend(sampled_trajs_test)

            visitations = get_demo_oms_from_trajectories(
                learned_policy_trajs, state_dim=self.env.state_dim)
            all_visitations = get_demo_oms_from_trajectories(
                all_trajs, state_dim=self.env.state_dim)
            expert_visitations = get_demo_oms_from_trajectories(
                expert_policy_trajs, state_dim=self.env.state_dim)

            costs_train, _, ods_agou = calculate_expected_similarities_and_std(cost_model_from_reward_net(self._reward_net, env=self.env, precalculated_rewards_per_pure_pr=pure_rewards_per_pure_pr), sample_trajs=learned_policy_trajs,
                                                                               expert_trajs=expert_policy_trajs, profile_for_cost_model=(1.0, 1.0, 1.0), profile_criteria=BASIC_PROFILES, preprocessing=FeaturePreprocess.NORMALIZATION, return_ods=True)
            costs_test, _, ods_agou_test = calculate_expected_similarities_and_std(cost_model_from_reward_net(self._reward_net, env=self.env, precalculated_rewards_per_pure_pr=pure_rewards_per_pure_pr), sample_trajs=learned_policy_trajs_test,
                                                                                   expert_trajs=expert_policy_trajs_test, profile_for_cost_model=(1.0, 1.0, 1.0), profile_criteria=BASIC_PROFILES, preprocessing=FeaturePreprocess.NORMALIZATION, return_ods=True)

            for chosen_profile in set(bpr for i, bpr in enumerate(BASIC_PROFILES) if society_profile_distribution[i] > 0.0):
                mntensor_train_per_pr[chosen_profile] = profiled_aggregated_similarity_Agourogiannis_et_al_2023(
                    costs_train['sample'], costs_train['expert'], preference_weights=chosen_profile)
                mntensor_test_per_pr[chosen_profile] = profiled_aggregated_similarity_Agourogiannis_et_al_2023(
                    costs_test['sample'], costs_test['expert'], preference_weights=chosen_profile)

                rewards_per_pr[chosen_profile] = predicted_r_np

        self.optimizer.zero_grad()
        grad_norm = 0
        ctrainsamp = th.as_tensor(
            costs_train['sample'], device=self._reward_net.device, dtype=self._reward_net.dtype)
        ctrainexp = th.as_tensor(
            costs_train['expert'], device=self._reward_net.device, dtype=self._reward_net.dtype)
        ctestsamp = th.as_tensor(
            costs_test['sample'], device=self._reward_net.device, dtype=self._reward_net.dtype)
        ctestexp = th.as_tensor(
            costs_test['expert'], device=self._reward_net.device, dtype=self._reward_net.dtype)

        agou_tr = 1.0 - th.abs(1.0 - 1.0/(-self._reward_net.trained_profile_net(- th.maximum(ctrainsamp, ctrainexp) /
                               th.minimum(ctrainsamp, ctrainexp))))  # profiled_aggregated_similarity_Agourogiannis_et_al_2023(, )
        assert th.all(agou_tr >= 0.0)
        with th.no_grad():
            # profiled_aggregated_similarity_Agourogiannis_et_al_2023(, )
            agou_test = 1.0 - th.abs(1.0 - 1/(- self._reward_net.trained_profile_net(- th.maximum(
                ctestsamp, ctestexp)/th.minimum(ctestsamp, ctestexp))))
            ...
            jaccard_per_pr_expert_train, jaccard_expert_train_normal, jaccard_tr, ods_jaccard_tr = jaccard_similarity(cost_model_from_reward_net(
                self._reward_net, env=self.env, precalculated_rewards_per_pure_pr=pure_rewards_per_pure_pr), profile_for_cost_model=learned_policy_profile, sample_trajs=learned_policy_trajs, expert_trajs=expert_policy_trajs, profile_criteria=set(profiles), preprocessing=FeaturePreprocess.NORMALIZATION, return_per_od=True)
            jaccard_per_pr_expert_train, jaccard_expert_train_normal, jaccard_tst, ods_jaccard_tr = jaccard_similarity(cost_model_from_reward_net(
                self._reward_net, env=self.env, precalculated_rewards_per_pure_pr=pure_rewards_per_pure_pr), profile_for_cost_model=learned_policy_profile, sample_trajs=learned_policy_trajs_test, expert_trajs=expert_policy_trajs_test, profile_criteria=set(profiles), preprocessing=FeaturePreprocess.NORMALIZATION, return_per_od=True)
        ...
        if self.fd_lambda != 0.0:
            ...
            # * OVERLAP_USE*th.float_power((1.0-jaccard_tr), 1.0/OVERLAP_REFERENCE)
            weights = th.float_power((1.0-agou_tr), 1.0/COST_PREFERENCE)
        else:
        ...
        rewards = self._reward_net(None, None, obs_matrix_all_trajs,
                                   done_matrix_all_trajs).reshape_as(visitation_count_diffs)
        ...
        with th.no_grad():
            ...
            visitations_test = get_demo_oms_from_trajectories(
                learned_policy_trajs_test, state_dim=self.env.state_dim)
            expert_visitations_test = get_demo_oms_from_trajectories(
                expert_policy_trajs_test, state_dim=self.env.state_dim)

            visitation_count_diffs_abs_train = np.sum([np.abs(visitations[(
                od, learned_policy_profile)] - expert_visitations[(od, pr)]) for od, pr in zip(ods, profiles)], axis=1)
            visitation_count_diffs_abs_test = np.sum([np.abs(visitations_test[(
                od, learned_policy_profile)] - expert_visitations_test[(od, pr)]) for od, pr in zip(ods_test, profiles_test)], axis=1)
    ...
    def _train_step_value_learning(self, obs_mat: th.Tensor, dones: th.Tensor = None, chosen_profile=(1.0, 0.0, 0.0), loss_weighting=1.0,  batch_size=None) -> Tuple[np.ndarray, np.ndarray]:
        ...
        th.autograd.set_detect_anomaly(False)
        learned_policy_profile = self._reward_net.get_learned_profile(
        ) if self.training_mode == TrainingModes.VALUE_SYSTEM_IDENTIFICATION else chosen_profile
        pure_rewards_per_pure_pr = dict()
        for pr in BASIC_PROFILES:
            pure_rewards_per_pure_pr[pr] = self.calculate_rewards_for_destinations_and_profile(
                self.destinations, pr, obs_mat=obs_mat, dones=dones)
        ...
        if batch_size is None:
            ...
            ods_index = np.random.choice(
                len(self.od_list_train), size=batch_size, replace=True)
            batch_od_train = [(od[0], od[1]) for od in np.array(
                self.od_list_train)[ods_index].tolist()]
        ...
        chosen_profile_expert_trajs_train = [
            t for od in batch_od_train for t in self.expert_trajectories_per_odpr[(od, chosen_profile)]]
        od_profiles_of_trajs_train = [
            ((t.infos[0]['orig'], t.infos[0]['des']), chosen_profile) for t in chosen_profile_expert_trajs_train]
        # chosen_profile_trajs_test = [t for t in chosen_profile_trajs if (t.infos[0]['orig'], t.infos[0]['des']) in self.od_list_test]
        # od_profiles_of_trajs_test = [((t.infos[0]['orig'], t.infos[0]['des']), chosen_profile) for t in chosen_profile_trajs_test]
        ...
        costs_train, agou, ods_agou = calculate_expected_similarities_and_std(cost_model_from_reward_net(self._reward_net, env=self.env, precalculated_rewards_per_pure_pr=pure_rewards_per_pure_pr), sample_trajs=sampled_trajs_train,
                                                                              expert_trajs=chosen_profile_expert_trajs_train, profile_for_cost_model=(1.0, 1.0, 1.0), profile_criteria=BASIC_PROFILES, preprocessing=FeaturePreprocess.NORMALIZATION, return_ods=True)
        ...
        costs_test, agou_test, ods_agou_test = calculate_expected_similarities_and_std(cost_model_from_reward_net(self._reward_net, env=self.env, precalculated_rewards_per_pure_pr=pure_rewards_per_pure_pr), sample_trajs=sampled_trajs_train,
                                                                                       expert_trajs=chosen_profile_expert_trajs_train, profile_for_cost_model=(1.0, 1.0, 1.0), profile_criteria=BASIC_PROFILES, preprocessing=FeaturePreprocess.NORMALIZATION, return_ods=True)

        mntensor_train = np.mean(profiled_aggregated_similarity_Agourogiannis_et_al_2023(
            costs_train['sample'], costs_train['expert'], preference_weights=chosen_profile))
        mntensor_test = np.mean(profiled_aggregated_similarity_Agourogiannis_et_al_2023(
            costs_test['sample'], costs_test['expert'], preference_weights=chosen_profile))
    ...
    def train(self, max_iter: int = 1000, render_partial_plots=True, training_mode=None, training_set_mode=None, batch_size=None, wait_until_end_of_iterations=False) -> np.ndarray:
        ...
        assert self.demo_state_om_per_profile[tuple(
            list(self.demo_state_om_per_profile.keys())[0])].shape == (len(obs_mat),)
        ...
        mean_absolute_difference_in_visitation_counts_per_profile_test = {
            pr: [] for pr in self.eval_profiles}
        mean_absolute_difference_in_visitation_counts_per_profile_train = {
            pr: [] for pr in self.eval_profiles}
        ...
        grad_norms_per_iteration_per_profile = {
            pr: [] for pr in self.eval_profiles}

        overlapping_proportions_per_iteration_per_profile_test = {
            pr: [] for pr in self.eval_profiles}
        overlapping_proportions_per_iteration_per_profile_train = {
            pr: [] for pr in self.eval_profiles}

        jaccard_pure_per_iteration_per_profile_test = {
            pr: [] for pr in self.eval_profiles}
        jaccard_pure_per_iteration_per_profile_train = {
            pr: [] for pr in self.eval_profiles}
        ...
        feature_expectation_differences_per_iteration_per_profile_test = {
            pr: [] for pr in self.eval_profiles}
        feature_expectation_differences_per_iteration_per_profile_train = {
            pr: [] for pr in self.eval_profiles}
        ...
        with networks.training(self._reward_net):
            ...
            for t in range(max_iter):
                # skip evaluating with a profile not being trained on (PROFILED_SOCIETY)
                skip_pr = collections.defaultdict(lambda: False)
                if t % 5 == 0 and self.stochastic_expert:
                    sampled_expert_trajectories = self.expert_policy.sample_trajectories(stochastic=self.stochastic_expert, repeat_per_od=self.n_repeat_per_od_monte_carlo,
                                                                                         with_profiles=BASIC_PROFILES if self.training_set_mode == TrainingSetModes.PROFILED_SOCIETY else self.training_profiles, od_list=self.od_list_train)
                    self._set_demo_oms_from_trajectories(
                ...
                if self.training_set_mode == TrainingSetModes.PROFILED_SOCIETY:
                    ...
                    od_train_per_basic_profile = self.divide_od_list_as_per_profile(
                        self.od_list_train, self.training_profiles[t % len(self.training_profiles)])

                    profile_proportions = np.asarray(self.training_profiles[t % len(
                        self.training_profiles)]) / np.sum(self.training_profiles[t % len(self.training_profiles)])
                    for pr, ods in od_train_per_basic_profile.items():
                        # If no examples for a certain profile, we do not eval and train  with it.
                        skip_pr[pr] = len(ods) < 1
                else:
                    od_train_per_basic_profile = {
                        pr: self.od_list_train for pr in self.training_profiles}
                ...
                if self.training_set_mode == TrainingSetModes.PROFILED_SOCIETY:
                    ...
                    learned_profiles_per_pr = {
                        pr: learned_policy_profile for pr in self.eval_profiles}
                ...
                for pr in [prs for prs in self.eval_profiles if not skip_pr[prs]]:
                    ...
                    feature_expectation_differences_per_iteration_per_profile_train[pr].append(
                        np.mean(fd_train_per_pr[pr]))

                    difference_in_visitation_counts_train = np.array([self.demo_state_om_per_profile[(
                        od, pr)] - visitations[(od, learned_profiles_per_pr[pr])] for od in self.od_list_train])
                    absolute_difference_in_visitation_counts_train = np.abs(
                        difference_in_visitation_counts_train)

                    absolute_difference_in_visitation_counts_per_od_train = np.sum(
                        absolute_difference_in_visitation_counts_train, axis=1)
                    # max_delta_per_od = np.max(absolute_difference_in_visitation_counts, axis=-1)

                    mean_absolute_difference_in_visitation_counts_train = np.mean(
                        absolute_difference_in_visitation_counts_per_od_train)

                    grad_norms_per_iteration_per_profile[pr].append(
                        grad_norm_per_pr[pr])
                    mean_absolute_difference_in_visitation_counts_per_profile_train[pr].append(
                        mean_absolute_difference_in_visitation_counts_train)

                    # TEST
                    feature_expectation_differences_per_iteration_per_profile_test[pr].append(
                        np.mean(fd_test_per_pr[pr]))

                    difference_in_visitation_counts_test = np.array([self.demo_state_om_per_profile[(
                        od, pr)] - visitations[(od, learned_profiles_per_pr[pr])] for od in self.od_list_test])
                    absolute_difference_in_visitation_counts_test = np.abs(
                        difference_in_visitation_counts_test)

                    # max_delta_per_od = np.max(absolute_difference_in_visitation_counts, axis=-1)
                    absolute_difference_in_visitation_counts_per_od_test = np.sum(
                        absolute_difference_in_visitation_counts_test, axis=1)

                    mean_absolute_difference_in_visitation_counts_test = np.mean(
                        absolute_difference_in_visitation_counts_per_od_test)

                    mean_absolute_difference_in_visitation_counts_per_profile_test[pr].append(
                        mean_absolute_difference_in_visitation_counts_test)

                if self.training_set_mode == TrainingSetModes.PROFILED_SOCIETY:

                    mean_absolute_difference_in_visitation_counts_per_profile_train['learned'].append(
                        historic_avg_vc[-1])

                    mean_absolute_difference_in_visitation_counts_per_profile_test['learned'].append(
                        historic_avg_vc_test[-1])
                ...
                if self.use_dijkstra:
                    ...
                    sampler: SimplePolicy = SimplePolicy.from_policy_matrix(
                        pi_per_profile, real_env=self.env)

                sampled_trajs = sampler.sample_trajectories(stochastic=False, repeat_per_od=1, with_profiles=[
                                                            prs for prs in self.eval_profiles if not skip_pr[prs]], od_list=self.env.od_list_int)
                ...
                overlapping_proportion_test = {pr: 0 for pr in [
                    prs for prs in self.eval_profiles if not skip_pr[prs]]}
                overlap_sorted_test = {pr: [] for pr in [
                    prs for prs in self.eval_profiles if not skip_pr[prs]]}

                overlapping_proportion_train = {pr: 0 for pr in [
                    prs for prs in self.eval_profiles if not skip_pr[prs]]}
                overlap_sorted_train = {pr: [] for pr in [
                    prs for prs in self.eval_profiles if not skip_pr[prs]]}

                jaccards_per_pr_train = {pr: [] for pr in [
                    prs for prs in self.eval_profiles if not skip_pr[prs]]}
                jaccards_per_pr_test = {pr: [] for pr in [
                    prs for prs in self.eval_profiles if not skip_pr[prs]]}
                avg_jaccard_per_pr_test = {pr: 0 for pr in [
                    prs for prs in self.eval_profiles if not skip_pr[prs]]}
                avg_jaccard_per_pr_train = {pr: 0 for pr in [
                    prs for prs in self.eval_profiles if not skip_pr[prs]]}
                ...
                for pr in [prs for prs in self.eval_profiles if not skip_pr[prs]]:
                    ...
                    sampled_trajs_of_pr = [
                        t for t in sampled_trajs if t.infos[0]['profile'] == pr]
                        ...
                        if odpr[1] == pr:
                            for traj in [t for t in sampled_trajs_of_pr if (t.infos[0]['orig'], t.infos[0]['des']) == odpr[0]]:
                                ...
                                if odpr[0] in self.od_list_test:
                                    ...
                                    for expert_traj in expert_trajs:
                                        if (expert_traj.infos[0]['orig'], expert_traj.infos[0]['des']) in self.od_list_test:
                                            ...
                                            for i in range(lexpert):
                                                if i < len(traj):
                                                    do_overlap = (
                                                        1 if expert_traj.obs[i] == traj.obs[i] else 0)
                                    ...
                                    _, _, jreal, jaccard_dist = jaccard(
                                        traj, expert_traj, cost_model=self.env.cost_model, profile=pr, preprocessing=FeaturePreprocess.NORMALIZATION)
                                    overlap_of_this_pr_test.append(
                                        overlapping_nodes/len(expert_traj))

                                    jaccards_per_pr_test[pr].append(
                                        jaccard_dist)
                                    avg_jaccard_per_pr_test[pr] += jaccard_dist/len(
                                        self.od_list_test)
                                ...
                                if odpr[0] in self.od_list_train:
                                    ...
                                    _, _, jreal, jaccard_dist = jaccard(
                                        traj, expert_traj, cost_model=self.env.cost_model, profile=pr, preprocessing=FeaturePreprocess.NORMALIZATION)
                                    overlap_of_this_pr_train.append(
                                        overlapping_nodes/len(expert_traj))

                                    # overlap_of_this_pr_train.append(overlapping_nodes/len(expert_traj))
                                    jaccards_per_pr_train[pr].append(
                                        jaccard_dist)
                                    avg_jaccard_per_pr_train[pr] += jaccard_dist/len(
                                        self.od_list_train)

                    overlapping_proportion_test[pr] /= total_possible_overlaps_of_this_pr_test

                    npoverlap_test = np.array(overlap_of_this_pr_test)
                    sorted_overlap_indices_test[pr] = np.argsort(
                        npoverlap_test)
                    # print(sorted_overlap_indices)
                    overlap_sorted_test[pr] = npoverlap_test[sorted_overlap_indices_test[pr]]

                    overlapping_proportions_per_iteration_per_profile_test[pr].append(
                        overlapping_proportion_test[pr])
                    jaccard_pure_per_iteration_per_profile_test[pr].append(
                        avg_jaccard_per_pr_test[pr])
                    ...
                    sorted_overlap_indices_train[pr] = np.argsort(
                        npoverlap_train)
                    # print(sorted_overlap_indices)
                    overlap_sorted_train[pr] = npoverlap_train[sorted_overlap_indices_train[pr]]

                    overlapping_proportions_per_iteration_per_profile_train[pr].append(
                        overlapping_proportion_train[pr])

                    jaccard_pure_per_iteration_per_profile_train[pr].append(
                        avg_jaccard_per_pr_train[pr])

                termination_condition_met = False
                if wait_until_end_of_iterations is False:
                    if self.training_set_mode != TrainingSetModes.PROFILED_SOCIETY:
                        termination_condition_met = (max(mean_absolute_difference_in_visitation_counts_per_profile_train[pr][-1] for pr in [prs for prs in self.eval_profiles if not skip_pr[prs]]) <= self.mean_vc_diff_eps or max(grad_norms_per_iteration_per_profile[pr][-1] for pr in [
                                                     prs for prs in self.eval_profiles if not skip_pr[prs]]) <= self.grad_l2_eps) and self.overlaping_percentage <= min(overlapping_proportion_train[pr] for pr in [prs for prs in self.eval_profiles if not skip_pr[prs]])
                    else:
                        termination_condition_met = max(grad_norms_per_iteration_per_profile[pr][-1] for pr in [
                                                        prs for prs in self.eval_profiles if not skip_pr[prs]]) <= self.grad_l2_eps and historic_avg_jaccard[-1] >= self.overlaping_percentage
                if (self.log_interval is not None and 0 == (t % self.log_interval)) or termination_condition_met or (t + 1 == max_iter):
                    ...
                    self.logger.record("max mean_absolute_difference_in_visitation_counts", max(
                        mean_absolute_difference_in_visitation_counts_per_profile_train[pr][-1] for pr in [prs for prs in self.eval_profiles if not skip_pr[prs]]))
                    self.logger.record("weight_norm", weight_norm)
                    self.logger.record("max grad_norm", max(
                        grad_norms_per_iteration_per_profile[pr][-1] for pr in [prs for prs in self.eval_profiles if not skip_pr[prs]]))
                    ...
                    if self.training_mode == TrainingModes.VALUE_SYSTEM_IDENTIFICATION or self.training_mode == TrainingModes.SIMULTANEOUS:
                        ...
                        print("New Learned Profile: ",
                              self._reward_net.get_learned_profile())
                        self.logger.record("Previous Profile: ", [
                                           f"{p:0.2f}" for p in learned_policy_profile])
                        self.logger.record("New Learned Profile: ", [
                                           f"{p:0.2f}" for p in self._reward_net.get_learned_profile()])
                    self.logger.dump(t)
                    ...
                    if len(historic_avg_agou_cost) > 0:
                        ...
                        plt.fill_between([i for i in range(len(agou))], agou-std_agou,
                                         agou+std_agou, edgecolor='orange', alpha=0.1, facecolor='orange')
                        plt.fill_between([i for i in range(len(agou))], jaccard_metric-std_jaccard,
                                         jaccard_metric+std_jaccard, edgecolor='black', alpha=0.1, facecolor='black')

                        plt.legend()
                        plt.grid()
                        plt.savefig(
                            f'plots/Maxent_{self.name_method}_similarities_train_{self.training_mode.value}_via_{self.training_set_mode.value}.pdf')

                        plt.figure(figsize=(6, 6))
                        plt.title(
                            f"Test: Similarity metrics (assuming learned profile)")
                        plt.xlabel("Training Iteration")
                        plt.ylabel("Average similarity over OD pairs")
                        agou_test = np.asarray(historic_avg_agou_cost_test)
                        jaccard_metric_test = np.asarray(
                            historic_avg_jaccard_test)
                        std_agou_test = np.asarray(historic_std_agou_cost_test)
                        std_jaccard_test = np.asarray(
                            historic_std_jaccard_test)
                        ...
                        plt.fill_between([i for i in range(len(agou_test))], agou_test-std_agou_test,
                                         agou_test+std_agou_test, edgecolor='orange', alpha=0.1, facecolor='orange')
                        plt.fill_between([i for i in range(len(jaccard_metric_test))], jaccard_metric_test-std_jaccard_test,
                                         jaccard_metric_test+std_jaccard_test, edgecolor='black', alpha=0.1, facecolor='black')

                        plt.legend()
                        plt.grid()
                        plt.savefig(
                            f'plots/Maxent_{self.name_method}_similarities_test_{self.training_mode.value}_via_{self.training_set_mode.value}.pdf')

                        plt.figure(figsize=(6, 6))
                        plt.title(
                            f"Train: Total absolute difference in visitation counts")
                        plt.xlabel("Training Iteration")
                        ...
                        plt.fill_between([i for i in range(len(
                            agou))], tvc-std_tvc, tvc+std_tvc, edgecolor='magenta', alpha=0.1, facecolor='magenta')

                        plt.legend()
                        plt.grid()
                        plt.savefig(
                            f'plots/Maxent_{self.name_method}_differences_train_{self.training_mode.value}_via_{self.training_set_mode.value}.pdf')

                        plt.figure(figsize=(6, 6))
                        plt.title(
                            f"Test: Total absolute difference in visitation counts")
                        plt.xlabel("Training Iteration")
                        plt.ylabel("Average TVC over OD pairs")
                        ...
                        plt.fill_between([i for i in range(len(tvc_test))], tvc_test-std_tvc_test,
                                         tvc_test+std_tvc_test, edgecolor='magenta', alpha=0.1, facecolor='magenta')

                        plt.legend()
                        plt.grid()
                        plt.savefig(
                            f'plots/Maxent_{self.name_method}_differences_test_{self.training_mode.value}_via_{self.training_set_mode.value}.pdf')
                    ...
                    plt.subplot(
                        2, max(3, len([prs for prs in self.eval_profiles if not skip_pr[prs]])), 1)
                    ...
                    plt.title(
                        f"Average trajectory jaccard similarity with profiles")
                    plt.xlabel("Training Iteration")
                    plt.ylabel(
                        "Average trajectory jaccard similarity with profiles")
                    for _, pr in enumerate([prs for prs in self.eval_profiles if not skip_pr[prs]]):
                        # plt.plot(mean_absolute_difference_in_visitation_counts_per_profile[pr], color=mean_absolute_difference_in_visitation_counts_per_profile_colors[pi], label='Maximum difference in visitation counts')
                        # print(overlapping_proportions_per_iteration_per_profile_test[pr])
                    ...
                    plt.subplot(
                        2, max(3, len([prs for prs in self.eval_profiles if not skip_pr[prs]])), 2)
                    plt.title(
                        f"Mean absolute difference in visitation counts by profile")
                    plt.xlabel("Training Iteration")
                    ...
                    for _, pr in enumerate([prs for prs in self.eval_profiles if not skip_pr[prs]]):
                        ...
                        plt.plot(mean_absolute_difference_in_visitation_counts_per_profile_test[pr],
                                 color=mean_absolute_difference_in_visitation_counts_per_profile_colors.get(
                                     pr, 'black'),
                    ...
                    plt.subplot(
                        2, max(3, len([prs for prs in self.eval_profiles if not skip_pr[prs]])), 3)
                    plt.title(
                        f"Expected trajectory profiled cost similarity per iteration")
                    plt.xlabel("Training Iteration")
                    ...
                    for pindex, pr in enumerate([prs for prs in self.eval_profiles if not skip_pr[prs]]):
                        plt.subplot(2, max(
                            3, len([prs for prs in self.eval_profiles if not skip_pr[prs]])), pindex+4)
                        plt.bar(range(len(
                            overlap_sorted_test[pr])), overlap_sorted_test[pr], color=overlapping_colors.get(pr, 'black'))
                        plt.xlabel('OD pairs')
                        plt.ylabel(
                            'Best overlap proportion with an expert trajectory')
                        plt.title(
                            f'Best overlap proportion with {BASIC_PROFILE_NAMES.get(pr, str(tuple([float(f"{l:0.2f}") for l in pr])))} agent per OD')
                        plt.xticks(range(len(overlap_sorted_test[pr])), labels=[str(od) for od in np.asarray(self.od_list_test)[
                                   sorted_overlap_indices_test[pr]]], rotation=90, fontsize=2)  # Setting x-axis ticks
                        # Highlighting the minimum value
                        plt.axvline(x=0, color='blue', linestyle='--',
                                    label=f'Worst Overlap: {float(np.min(overlap_sorted_test[pr])):0.2f}\n(at {np.asarray(self.od_list_test)[sorted_overlap_indices_test[pr][0]]}).\nOverlap Proportion: {overlapping_proportion_test[pr]:0.2f}')
                        plt.legend()
                        # plt.grid()
                    ...
                    plt.subplot(
                        2, max(3, len([prs for prs in self.eval_profiles if not skip_pr[prs]])), 2)
                    plt.title(
                        f"Expected visitation count difference per profile")
                    plt.xlabel("Training Iteration")
                    plt.ylabel(
                        "Expected absolute difference in visitation counts")
                    for _, pr in enumerate([prs for prs in self.eval_profiles if not skip_pr[prs]]):
                    ...
                    for pindex, pr in enumerate([prs for prs in self.eval_profiles if not skip_pr[prs]]):
                        plt.subplot(2, max(
                            3, len([prs for prs in self.eval_profiles if not skip_pr[prs]])), pindex+4)
                        plt.bar(range(len(
                            overlap_sorted_train[pr])), overlap_sorted_train[pr], color=overlapping_colors.get(pr, 'black'))
                        plt.xlabel('OD pairs')
                        plt.ylim(top=1.1)
                        plt.ylabel(
                            'Best overlap proportion with an expert trajectory')
                        plt.title(
                            f'Best overlap proportion with {BASIC_PROFILE_NAMES.get(pr, str(tuple([float(f"{l:0.2f}") for l in pr])))} agent per OD')
                        plt.xticks(range(len(overlap_sorted_train[pr])), labels=[str(od) for od in np.asarray(self.od_list_train)[
                                   sorted_overlap_indices_train[pr]]], rotation=90, fontsize=2)  # Setting x-axis ticks
                        # Highlighting the minimum value
                        plt.axvline(x=0, color='blue', linestyle='--',
                                    label=f'Worst Overlap: {float(np.min(overlap_sorted_train[pr])):0.2f}\n(at {np.asarray(self.od_list_train)[sorted_overlap_indices_train[pr][0]]}).\nOverlap Proportion: {overlapping_proportion_train[pr]:0.2f}')
                        plt.legend()
                        # plt.grid()
                    ...
                    for pr in [prs for prs in self.eval_profiles if not skip_pr[prs]]:
                        ...
                        if render_partial_plots:  # RENDER WORST OVERLAP
                            ...
                            sampler: SimplePolicy = SimplePolicy.from_sb3_policy(
                                self.policy, real_env=self.env)
                            path, edge_path = sampler.sample_path(
                                start=orig, des=des, stochastic=False, profile=pr, t_max=1000)
                            expert_sampler: SimplePolicy = SimplePolicy.from_trajectories(
                                self.expert_trajectories, real_env=self.env)
                            expert_path, expert_edge_path = expert_sampler.sample_path(
                                start=orig, des=des, stochastic=False, profile=pr, t_max=1000)
                            print(
                                f"Learned path from {orig} to {des}", edge_path)
                            print(
                                f"Expert path from {orig} to {des}", expert_edge_path)
                            # print("PRNP", predicted_r_np)
                            self.env.render(caminos_by_value={'sus': [path,], 'eff': [
                                            expert_path]}, file=f"test_me_partial_{t}_{pr}_train.png", show=False, show_edge_weights=False)
