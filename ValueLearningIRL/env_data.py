...
from src.envs.tabularVAenv import encrypt_state
...
from src.vsl_policies import VAlignedDictSpaceActionPolicy, LearnerValueSystemLearningPolicy, profile_sampler_in_society, random_sampler_among_trajs, sampler_from_policy
...
def custom_cost_from_reward(environment: RoadWorldGymPOMDP, reward, state_des, profile):
    rews = [reward[s, a] for a, s in zip(
        *environment.pre_acts_and_pre_states[state_des[0]]) if s != environment.cur_des]
    state_acts = [(s, a) for a, s in zip(
        *environment.pre_acts_and_pre_states[state_des[0]]) if s != environment.cur_des]
    if len(rews) == 0.0:
...
class EnvDataForIRL():
    ...
    def get_reward_net(self, algorithm='me'):
        ...
        action_features_extractor_kwargs = dict(
            n_categories=self.env.action_dim)
    ...
    def ad_train_config(self):
        ...
        base['vgl'].update(dict(
            max_iter=1000,
            # 1000 was too few with full variable length trajectories
            n_seeds_for_sampled_trajectories=self.n_seeds_total,
            n_sampled_trajs_per_seed=self.n_expert_samples_per_seed,
...
class EnvDataForIRLFireFighters(EnvDataForIRL):
    ...
    def __init__(self, env_name, discount_factor=1.0, feature_selection=DEFAULT_FEATURE_SELECTION, horizon=DEFAULT_HORIZON, is_society=False, initial_state_dist=DEFAULT_INITIAL_STATE_DISTRIBUTION, learn=False, use_pmovi_expert=USE_PMOVI_EXPERT, n_seeds_for_samplers=DEFAULT_N_SEEDS,
        ...
        self.sampler_over_precalculated_trajs = sampler_over_precalculated_trajs
        self.horizon = horizon
        self.n_seeds_total = n_seeds_for_samplers
        self.target_align_func_sampler = profile_sampler_in_society if is_society else lambda al_func: al_func
        ...
        if self.approx_expert is False:
            no_previous = False
            try:
                expert_policy_train = LearnerValueSystemLearningPolicy.load(
                    ref_env=env_training, path=EnvDataForIRLFireFighters.POLICY_SAVE_PATH)
                ...
                if self.retrain or no_previous:
                    ...
                    for alignment in profiles:
                        timesteps = 1000000
                        expert_policy_train.learn(
                            alignment_function=alignment, total_timesteps=timesteps, tb_log_name=f'EXPERT_PPO_{alignment}_{timesteps}')
                        expert_policy_train.save(
                            path=EnvDataForIRLFireFighters.POLICY_SAVE_PATH)
        ...
        self.initial_state_distribution_for_expected_alignment_eval = np.zeros(
            (self.env.n_states,), dtype=np.float64)
        ...
        if self.sampler_over_precalculated_trajs:
            ...
            initial_states_of_expert_trajs = [
                t.obs[0] for t in expert_trajs_train]
            initial_state_dist_expert_trajs = np.zeros_like(
                self.env.initial_state_dist)
            initial_state_dist_expert_trajs[initial_states_of_expert_trajs] = 1/len(
                initial_states_of_expert_trajs)
            initial_state_dist_intersected = self.env.initial_state_dist * \
                initial_state_dist_expert_trajs
            initial_state_dist_intersected /= np.sum(
                initial_state_dist_intersected)
            assert np.allclose(np.sum(initial_state_dist_intersected), 1.0)

            self.env.set_initial_state_distribution(
                initial_state_dist_intersected)
            assert len(np.where(initial_state_dist_intersected > 0.0)[
                       0]) <= self.n_seeds_total
            expert_policy_train.env.set_initial_state_distribution(
                initial_state_dist_intersected)
    ...
    def set_defaults(self):
        ...
        self.vsi_optimizer_kwargs = {
            "lr": 0.01, "weight_decay": 0.0000}  # ME DEMO_OM_FALSE
        ...
        # IN ME THIS IS NOT USED, SEE CUSTOM_OPTIMIZER_KWARGS.
    ...
    def pc_train_config(self):
        ...
        base['vgl'].update(dict(
            ...
            # 15,5 | 4,1: PREV 40.
            initial_epoch_multiplier=40, transition_oversampling=1
        ))
        base['vsi'].update(dict(
            max_iter=10000,
            random_trajs_proportion=0.8,
        ...
        if self.use_one_hot_state_action:
            ...
            def processing_obs(torch_obs):
                states, actions = calculate_s_trans_ONE_HOT_FEATURES(
                    torch_obs, self.env.real_env.state_space, self.env.real_env.action_space)
    ...
    def ad_config(self):
        ...
        if base['vsi']['learner_class'] == PPO:
            ...
            new_policy_kwargs = dict(
                features_extractor_class=OneHotFeatureExtractor,
                features_extractor_kwargs=dict(
                    n_categories=self.env.state_dim),
                net_arch=[],  #  net arch is input to output linearly,
            )
            """new_policy_kwargs=dict(
                features_extractor_class=ObservationMatrixFeatureExtractor,
...
class EnvDataForRoadWorld(EnvDataForIRL):
    ...
    def __init__(self, env_name=ROAD_WORLD_ENV_NAME, horizon=DEFAULT_HORIZON, dest=DEFAULT_DEST, n_seeds_for_samplers=DEFAULT_N_SEEDS,
        ...
        for w in profiles:
            ...
            if self.approx_expert == False:
                if self.retrain:
                    assumed_expert_pi = self.compute_precise_policy(
                        env_real, w, reward)
                    ...
                    np.save(
                        f'roadworld_env_use_case/expert_policy_{w}.npy', assumed_expert_pi)
                else:
                    try:
        ...
        for tr in expert_trajs_train:
            if self.expose_observations == False and self.stochastic_expert == False:
                assert tr.obs[-1] == self.dest
        if self.sampler_over_precalculated_trajs:
            for tr in expert_trajs_train:
                if self.stochastic_expert == False:
                    assert tr.obs[-1] == self.dest
            self.vgl_expert_train_sampler = partial(
                partial(random_sampler_among_trajs, replace=False), expert_trajs_train)
            self.vsi_expert_train_sampler = partial(
    ...
    def compute_precise_policy(self, env_real: FixedDestRoadWorldGymPOMDP, w=None, reward=None):
        ...
        if reward is not None:
            def custom_cost(stdes, profile): return custom_cost_from_reward(
                environment, -reward, stdes, profile)
        policy = np.zeros(
            (env_real.state_dim, env_real.action_dim), dtype=np.float32)
        ...
        for s in np.arange(env_real.state_dim):
            ...
            av_actions = np.asarray(
                env_real.real_environ.get_available_actions_from_state(edge_state))
            ...
            if actions is None:
                if edge_state[0] == edge_state[1]:
                    actions = environment.get_available_actions_from_state(
                        (edge_state[1], edge_state[1]))
                    if profile is not None:
                        self._state_to_action[(
                            (edge_state[1], edge_state[1]), profile)] = actions
                    ...
                    for iedge in range(1, len(path)):
                        ...
                        av_actions_prev = np.asarray(
                            environment.get_available_actions_from_state(prev_state))
                        av_states = [environment.get_state_des_transition(
                            prev_state, av_c) for av_c in av_actions_prev]
        ...
        policy = concentrate_on_max_policy(
            policy, valid_action_checker=lambda s: env_real.valid_actions(s, None))
        if __debug__:
    ...
    def pc_train_config(self):
        ...
        base['vgl'].update(dict(
            max_iter=10000,
            # 1000 was too few with full variable length trajectories
            n_seeds_for_sampled_trajectories=2500,
            n_sampled_trajs_per_seed=1,
    ...
    def ad_config(self):
        ...
        base['vsi']['policy_class'] = 'MlpPolicy'
        base['vgl']['policy_class'] = 'MlpPolicy'
        # will use the MaskedPPO library. FALSE is better with AIRL
        base['vsi']['masked_policy'] = False
        base['vgl']['masked_policy'] = False
    ...
    def ad_train_config(self):
        ...
        base['vgl'].update(dict(
            ...
            n_disc_updates_per_round=2,
            custom_optimizer_kwargs={
                "lr": 0.005, "weight_decay": 0.0000, 'betas': (0.0, 0.0)}
    ...
    def set_defaults(self):
        ...
        self.vsi_optimizer_kwargs = {
            "lr": 0.005, "weight_decay": 0.0000, 'betas': (0.0, 0.0)}  #  if AIRL
        ...
        self.vgl_optimizer_kwargs = {
            "lr": 0.05, "weight_decay": 0.0000, 'betas': (0.0, 0.0)}  #  if AIRL
