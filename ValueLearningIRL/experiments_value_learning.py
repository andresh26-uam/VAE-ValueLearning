...
plot_avg_value_matrix(averages_matrix, std_matrix, file='plots/expected_value_matrix.pdf')
...
plt.subplot(2, 4,2)
plt.title(f"TRAIN: Average edit distance with profiles")
plt.xlabel("Training Iteration")
plt.ylabel("Average trajectory edit distance with expert")
for _, pr in enumerate(EXAMPLE_PROFILES):
    #plt.plot(mean_absolute_difference_in_visitation_counts_per_profile[pr], color=mean_absolute_difference_in_visitation_counts_per_profile_colors[pi], label='Maximum difference in visitation counts')
    #print(overlapping_proportions_per_iteration_per_profile_test[pr])
    avg = average_train_stats[pr]['ed']
    std_dev = np.sqrt(std_train_stats[pr]['ed'])
...
plt.subplot(2, 4,2+4)
plt.title(f"TEST: Average edit distance with profiles")
plt.xlabel("Training Iteration")
plt.ylabel("Average trajectory edit distance with expert")
for _, pr in enumerate(EXAMPLE_PROFILES):
    #plt.plot(mean_absolute_difference_in_visitation_counts_per_profile[pr], color=mean_absolute_difference_in_visitation_counts_per_profile_colors[pi], label='Maximum difference in visitation counts')
    #print(overlapping_proportions_per_iteration_per_profile_test[pr])
    avg = average_test_stats[pr]['ed']
    std_dev = np.sqrt(std_test_stats[pr]['ed'])
...
for _, pr in enumerate(EXAMPLE_PROFILES):
    ...
    std_dev = np.sqrt(std_train_stats[pr]['fd'])
...
for _, pr in enumerate(EXAMPLE_PROFILES):
    ...
    std_dev = np.sqrt(std_test_stats[pr]['fd'])
    plt.plot(avg,
                color=PROFILE_COLORS[pr], 
                label=f'Pr: \'{BASIC_PROFILE_NAMES[pr]}\'\nLast: {float(avg[-1]):0.3f}'
                )
...
plt.savefig(f'plots/ME_experiment_results_{N_EXPERIMENTS}x{N_ITER_PER_EXPERIMENT}.pdf')
